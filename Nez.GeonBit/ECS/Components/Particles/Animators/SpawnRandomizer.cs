...
{
    private readonly Vector3? _impulseDirection;
    private readonly Vector3? _impulseDirectionJitter;
    private readonly float? _maxAlpha;
    private readonly Color? _maxColor;
    private readonly float? _maxImpulseStrength;
    private readonly float? _maxScale;

    private readonly Vector3? _maxScaleVector;

    // all jitters
    private readonly float? _minAlpha;
    private readonly Color? _minColor;
    private readonly float? _minImpulseStrength;
    private readonly float? _minScale;
    private readonly Vector3? _minScaleVector;
    private readonly Vector3? _positionJitter;
    private readonly Vector3? _rotationJitter;
    ...
    public override void OnAddedToEntity()
    {
        ...
        if (_minAlpha != null)
        {
            var alpha = _minAlpha.Value + Random.NextFloat() * (_maxAlpha.Value - _minAlpha.Value);
            foreach (var target in targets) target.MaterialOverride.Alpha = alpha;
        }

        // random scale
        if (_minScale != null)
        {
            var scale = _minScale.Value + Random.NextFloat() * (_maxScale.Value - _minScale.Value);
            Node.Scale *= scale;
        }
        ...
        if (_minColor != null)
        {
            ...
            foreach (var target in targets) target.MaterialOverride.DiffuseColor = color;
        }

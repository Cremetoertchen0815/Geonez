...
public struct BaseAnimatorProperties
{
    ...
    ///     Default properties.
    /// </summary>
    public static BaseAnimatorProperties Defaults = new(speedFactor: 1f);
    ...
    /// <summary>
    ///     If this animator operates on components (in oppose to Scene GeonNode), filter which components to operate on by
    ///     name.
    /// </summary>
    public string FilterTargetsByName { get; }
...
public abstract class BaseAnimator : GeonComponent
{
    ...
    protected BaseRendererComponent[] Renderables
    {
        get
        {
            ...
            var list = Entity.GetInternalData(ref AnimatorRenderablesInternalKey);
            if (list == null)
            {
    ...
    protected BaseRendererWithOverrideMaterial[] ModelRenderables
    {
        ...
        {
            // create list of renderables to operate on (only happens first time)
            var list = Entity.GetInternalData(ref AnimatorModelsInternalKey);
            if (list == null)
            {
                list = Entity.GetComponents<BaseRendererWithOverrideMaterial>(BaseProperties.FilterTargetsByName)
                    .ToArray();
    ...
    {
        return AnimatorUtils.RandDirection(baseVector, randDir);
    }
    ...
    {
        return AnimatorUtils.RandColor(baseColor, randColor);
    }
    ...
    {
        // increase time alive
        var timeIncreased = Time.DeltaTime * BaseProperties.SpeedFactor;
        TimeAlive += timeIncreased;

        // if there's initial delay that need to expire stop here
        if (TimeAlive < BaseProperties.DelayToStart) return;

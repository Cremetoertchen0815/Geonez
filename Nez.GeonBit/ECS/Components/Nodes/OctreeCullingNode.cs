...
internal class OctreeCullingNode : Node
{
    ...
    /// </summary>
    private readonly uint _divisionsLeft;

    private readonly Vector3 _indexInParent;
    ...
    private readonly List<Node> _nodesUnderThisOctreeBox = new();

    // octree settings (shared between all octree nodes inside the tree).
    private readonly OctreeData _octreeData;

    // parent pointer and index in parent children
    private readonly OctreeCullingNode _parentOctree;
    ...
    /// </summary>
    private BoundingBox[,,] _childBoundingBoxes;

    /// <summary>
    ///     The child octrees under this octree node.
    /// </summary>
    private OctreeCullingNode[,,] _childOctrees;
    ...
    private int _countToRemoveSelf;

    /// <summary>
    ///     Bounding box entity for debug rendering.
    /// </summary>
    private BoundingBoxEntity _debugBoundingBoxEntity;
    ...
    ///     The starting bounding box of the entire octree. GeonNode: nodes that are outside this box
    ///     will not be culled properly.
    /// </param>
    ...
    private void InitOctreeBox(BoundingBox boundingBox)
    {
        ...
        if (!IsOctreeRoot) Transformations = null;
    ...
    protected void AddToSubTree(Node node, int x, int y, int z)
    {
        ...
        if (_childOctrees[x, y, z] == null)
            _childOctrees[x, y, z] = new OctreeCullingNode(this, new Vector3(x, y, z), _childBoundingBoxes[x, y, z],
                _divisionsLeft - 1);
    ...
    protected void RemoveFromTree(Node node)
    {
        ...
        foreach (var linked in node.LinkedNodes)
        {
            ...
            if (octreeNode != null) octreeNode.RemoveFromSelf(node);
        }
    }
    ...
    private void RemoveFromSelf(Node node)
    {
        ...
        if (Empty && _nodesUnderThisOctreeBox.Count == 0 && !HaveSubTrees) _countToRemoveSelf = 1;
    }
    ...
    protected override void DoTransformationsUpdateIfNeeded()
    {
        ...
        if (IsOctreeRoot) base.DoTransformationsUpdateIfNeeded();
    }
    ...
    protected void DrawOctreeNodeAfterVisibleCheck(bool forceEvenIfAlreadyDrawn = false)
    {
        ...
        if (DebugRenderOctreeParts)
        {
            ...
            _debugBoundingBoxEntity.BoxEffect.DiffuseColor = _nodesUnderThisOctreeBox.Count > 0
        ...
        foreach (var child in _nodesUnderThisOctreeBoxArray) child.Draw(forceEvenIfAlreadyDrawn);

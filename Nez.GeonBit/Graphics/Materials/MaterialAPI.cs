...
namespace Nez.GeonBit.Materials
{
	...
	public abstract class MaterialAPI
	{
		...
		{
			get => _fogEnabled;
			set { _fogEnabled = value; SetAsDirty(MaterialDirtyFlags.Fog); }
		}
		...
		private uint _viewMatrixVersion = 0;

		// local projection matrix version
		private uint _projectionMatrixVersion = 0;
		...
		protected void SetAsDirty(int val) => _dirtyFlags |= val;

		/// <summary>
		/// Add to dirty flags.
		/// </summary>
		/// <param name="val">Value to add to dirty flags using the or operator.</param>
		protected void SetAsDirty(MaterialDirtyFlags val) => _dirtyFlags |= (int)val;

		/// <summary>
		/// Check dirty flags.
		/// </summary>
		/// <param name="val">Value to test if dirty.</param>
		protected bool IsDirty(int val) => (_dirtyFlags & val) != 0;

		/// <summary>
		/// Add to dirty flags.
		/// </summary>
		/// <param name="val">Value to add to dirty flags using the or operator.</param>
		protected bool IsDirty(MaterialDirtyFlags val) => (_dirtyFlags & (int)val) != 0;
		...
		public virtual void SetBoneTransforms(Matrix[] bones) => throw new System.InvalidOperationException("Material does not support bone transformations in GPU.");

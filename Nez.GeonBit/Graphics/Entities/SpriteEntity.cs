...
public class SpriteEntity : BaseRenderableEntity
{
    ...
    private static readonly Dictionary<string, MaterialAPI> _sharedMaterials = new();

    // currently playing animation (or null, if not playing any clips at the momeny)
    private SpriteAnimationClipPlay _currAnimation;
    ...
    public SpriteEntity(SpriteSheet spritesheet, MaterialAPI material)
    {
    ...
    /// <param name="useSharedMaterial">
    ///     If true, will use a shared material for all sprites with this texture. If false, will
    ///     create a new material for this specific sprite instance.
    /// </param>
    ...
    public void CopyStep(SpriteEntity other)
    {
        ...
        if (other._currAnimation != null)
            PlayAnimation(other._currAnimation.Clip, other._currAnimation.SpeedFactor,
                other._currAnimation.CurrentStep);
    }
    ...
    /// <returns></returns>
    private static MaterialAPI GetCtorMaterial(Texture2D texture, bool useSharedMaterial)
    {
        ...
        if (useSharedMaterial)
        {
            ...
            if (_sharedMaterials.TryGetValue(texture.Name, out var material)) return material;
        ...
        {
            // create material
            MaterialAPI material = new SpriteMaterial(new AlphaTestEffect(Core.GraphicsDevice))
            {
    ...
    public void Update(float elapsedTime)
    {
        ...
        {
            var index = _currAnimation.Update(elapsedTime);
            _spritesheetStep = Spritesheet.GetStep(index);
        }
    }
        ...
        ref Matrix worldTransformations)
    {
        ...
        // get bounding sphere
        return new BoundingSphere(position, Math.Max(scale.X, scale.Y));
    }
}
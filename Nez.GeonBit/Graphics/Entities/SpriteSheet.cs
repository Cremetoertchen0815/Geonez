...
public class SpriteSheet
{
    ...
    private readonly List<SpriteSheetStep> _steps = new();

    // dictionary of step names (to get sprite from spritesheet via string identifier)
    private readonly Dictionary<string, int> _stepsIdentifierToIndex = new();
    ...
    {
        if (stepsCount != null) BuildForConstStepSize(stepsCount.Value);
    }
    ...
    public void AddStep(Vector2 position, Vector2 size, string identifier = null)
    {
        ...
        if (identifier != null) _stepsIdentifierToIndex[identifier] = _steps.Count - 1;
    }
    ...
    public void BuildForConstStepSize(Point stepsCount)
    {
        ...
        // create all steps
        for (var j = 0; j < stepsCount.Y; ++j)
        for (var i = 0; i < stepsCount.X; ++i)
        {
    ...
    private SpriteSheetStep BuildStep(Vector2 positionInSpritesheet, Vector2 sizeInSpriteSheet)
    {
        ...
        // set normal
        for (var i = 0; i < 4; ++i) vertices[i].Normal = Vector3.Forward;
...
public class SpriteAnimationClip
{
    ...
    private readonly Dictionary<int, float> _perStepDelays = new();
    ...
    public SpriteAnimationClip(int startStep, int endStep, float speed = 1f, bool loop = true)
    {
        ...
        if (startStep > endStep)
            throw new InvalidOperationException(
                "Animation clip ending step index cannot be smaller than starting step index!");
    ...
    {
        var ret = 1f / Speed;
        if (_perStepDelays.TryGetValue(stepRelativeIndex, out var extra)) return ret + extra;
        return ret;
    }
...
public class SpriteAnimationClipPlay
{
    ...
    private float _timeForNextStep;
    ...
    ///     Animation clip.
    /// </summary>
    public SpriteAnimationClip Clip { get; }

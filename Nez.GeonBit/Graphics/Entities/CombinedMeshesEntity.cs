...
namespace Nez.GeonBit
{
	...
	public class CombinedMeshesEntity<VertexType> : BaseRenderableEntity, IShadowCaster where VertexType : struct, IVertexType
	{
		...
		{
			// sanity check - if build was called assert
			if (_wasBuilt) { throw new System.InvalidOperationException("Cannot add meshes to Combined Mesh Entity after it was built!"); }
			...
			for (int i1 = 0; i1 < mesh.MeshParts.Count; i1++)
			{
				...
				for (int i = 0; i < vertexBufferSize / sizeof(float); i += vertexStride / sizeof(float))
				{
					...
					switch (_vtype)
					{
						...
						default:
							throw new System.Exception("Forgot to implement type support?");
					}
		...
		private Vector3 GetPosition(VertexType vertex)
		{
			switch (_vtype)
			{
				...
				default:
					throw new System.Exception("Forgot to implement type?");
			}
		}

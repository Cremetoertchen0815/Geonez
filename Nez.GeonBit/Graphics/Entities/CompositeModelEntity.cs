...
{
    protected List<MeshEntity> _meshAccess = new();

    /// <summary>
    ///     Dictionary with all the mesh entities.
    /// </summary>
    protected OrderedDictionary _meshes = new();
    ...
    public List<MeshEntity> GetMeshes()
    {
        ...
        for (var i = 0; i < _meshAccess.Count; i++) ret.Add(_meshAccess[i]);
        return ret;
    }
    ...
    public List<MaterialAPI> GetMaterials()
    {
        ...
        for (var j = 0; j < _meshAccess.Count; j++)
        {
            var mesh = _meshAccess[j];
            for (var i = 0; i < mesh.Mesh.MeshParts.Count; ++i)
            {

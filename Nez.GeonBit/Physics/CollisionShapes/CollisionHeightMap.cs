...
public class CollisionHeightMap : ICollisionShape
{
    ...
    public CollisionHeightMap(byte[] heightData, Point size, Vector2 scale, float minHeight = 0f,
        float maxHeight = 100f, float heightScale = 1f, bool useDiamondSubdivision = false)
    {
        // build shape
    ...
    /// <param name="heightScale">Optional height scale.</param>
    /// <param name="useDiamondSubdivision">Divide the tiles into diamond shapes for more accurare results.</param>
    public CollisionHeightMap(Vector3[,] heightData, Vector2 scale, float minHeight = 0f, float maxHeight = 100f,
        float heightScale = 1f, bool useDiamondSubdivision = false)
    {
        // get size and convert to bytes array
        var size = new Point(heightData.GetLength(0), heightData.GetLength(1));
        var array = new float[size.X * size.Y * 4];
    ...
    /// <param name="heightScale">Optional height scale.</param>
    /// <param name="useDiamondSubdivision">Divide the tiles into diamond shapes for more accurare results.</param>
    public CollisionHeightMap(byte[,] heightData, Vector2 scale, float minHeight = 0f, float maxHeight = 100f,
        float heightScale = 1f, bool useDiamondSubdivision = false)
    {
        // get size and convert to bytes array
        var size = new Point(heightData.GetLength(0), heightData.GetLength(1));
    ...
    /// <param name="heightScale">Optional height scale.</param>
    /// <param name="useDiamondSubdivision">Divide the tiles into diamond shapes for more accurare results.</param>
    public CollisionHeightMap(float[] heightData, Point size, Vector2 scale, float minHeight = 0f,
        float maxHeight = 100f, float heightScale = 1f, bool useDiamondSubdivision = false)
    {
        // build shape
        Build(heightData, size, scale, minHeight, maxHeight, heightScale, 1, useDiamondSubdivision);
    ...
    /// <param name="heightScale">Optional height scale.</param>
    /// <param name="useDiamondSubdivision">Divide the tiles into diamond shapes for more accurare results.</param>
    public CollisionHeightMap(float[,] heightData, Vector2 scale, float minHeight = 0f, float maxHeight = 100f,
        float heightScale = 1f, bool useDiamondSubdivision = false)
    {
        // get size and convert to bytes array
        var size = new Point(heightData.GetLength(0), heightData.GetLength(1));
    ...
    {
        // free the int ptr
        if (_rawDataHandle != default) _rawDataHandle.Free();
    }
    ...
    /// <param name="useDiamondSubdivision">Divide the tiles into diamond shapes for more accurare results.</param>
    private void Build(byte[] heightData, Point size, Vector2 scale, float minHeight = 0f, float maxHeight = 100f,
        float heightScale = 1f, int upIndex = 1, bool useDiamondSubdivision = false)
    {
        // get int ptr for data bytes
        _rawDataHandle = GCHandle.Alloc(heightData, GCHandleType.Pinned);
        var address = _rawDataHandle.AddrOfPinnedObject();

        // create heightmap
        var heightmap = new HeightfieldTerrainShape(size.X, size.Y, address,
            heightScale, minHeight, maxHeight, upIndex,
            PhyScalarType.Byte, false);
    ...
    /// <param name="upIndex">Up index.</param>
    /// <param name="useDiamondSubdivision">Divide the tiles into diamond shapes for more accurare results.</param>
    private void Build(float[] heightData, Point size, Vector2 scale, float minHeight = 0f, float maxHeight = 100f,
        float heightScale = 1f, int upIndex = 1, bool useDiamondSubdivision = false)
    {
        // get int ptr for data bytes
        _rawDataHandle = GCHandle.Alloc(heightData, GCHandleType.Pinned);
    ...
    {
        throw new InvalidOperationException("Cannot clone height-map physical shape!");
    }
}
...
float4 ReflectAlongSurface(float3 Position, float3 Normal, float blurAmount) {
	
	float3 L = normalize(EyePosition - Position);
	float Diff = saturate(dot(L, Normal));
	
	// Calculate reflection vector
	float3 Reflect = reflect(-L, Normal);

	return texCUBElod(ReflectionCubeMapSampler, float4(Reflect, ReflectionBlur * blurAmount));
}
...
float4 PS(VertexShaderOutputN input) : COLOR
{
	...
	float4 albedo = tex2D(MainTextureSampler, input.TextureCoordinate.xy);
	float4 retColor = AlbedoEnabled ? albedo : 1.0f;
	retColor.a = 1;

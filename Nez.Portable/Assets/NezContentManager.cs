...
namespace Nez.Systems
{
	...
	public class NezContentManager : ContentManager
	{
		...
		public Texture2D LoadTexture(string name, bool premultiplyAlpha = false)
		{
			...
			if (LoadedAssets.TryGetValue(name, out object asset))
			{
		...
		/// </summary>
		public SpriteAtlas LoadSpriteAtlas(string name, bool generateOutline, bool premultiplyAlpha = false) => SpriteAtlasLoader.ParseSpriteAtlas(name, this, generateOutline, premultiplyAlpha);
		...
		{
			string cacheKey = typeof(T).Name + "-" + Utils.RandomString(5);
			var effect = new T
			{
				Name = cacheKey
			};
		...
		public T LoadEffect<T>(string name) where T : Effect
		{
			...
			byte[] bytes = EffectResource.GetFileResourceBytes(name);
		...
		public void UnloadAsset<T>(string assetName) where T : class, IDisposable
		{
			if (IsAssetLoaded(assetName))
			{
				try
				{
					...
					object assetToRemove = LoadedAssets[assetName];
		...
		internal string LogLoadedAssets()
		{
			...
			foreach (string asset in _loadedEffects.Keys)
		...
		public override void Unload()
		{
			...
			foreach (string key in _loadedEffects.Keys)

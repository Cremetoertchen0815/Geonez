...
public partial class TmxMap : TmxDocument
{
    ...
    public ITmxLayer GetLayer(string name)
    {
        return Layers.Contains(name) ? Layers[name] : null;
    }
    ...
    public T GetLayer<T>(int index) where T : ITmxLayer
    {
    ...
    /// <param name="name">Name.</param>
    /// <typeparam name="T">The 1st type parameter.</typeparam>
    public T GetLayer<T>(string name) where T : ITmxLayer
    {
    ...
    {
        return ObjectGroups.Contains(name) ? ObjectGroups[name] : null;
    }

...
namespace Nez.Tiled
{
	...
	public partial class TmxMap : TmxDocument
	{
		...
		public Point WorldToTilePosition(Vector2 pos, bool clampToTilemapBounds = true) => new Point(WorldToTilePositionX(pos.X, clampToTilemapBounds), WorldToTilePositionY(pos.Y, clampToTilemapBounds));
		...
		{
			int tileX = Mathf.FastFloorToInt(x / TileWidth);
		...
		{
			int tileY = Mathf.FloorToInt(y / TileHeight);
			if (!clampToTilemapBounds)
				return tileY;

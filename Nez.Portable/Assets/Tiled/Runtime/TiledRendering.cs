...
public static class TiledRendering
{
        ...
        RectangleF cameraClipBounds)
    {
        ...
        if (layer.Map.RequiresLargeTileCulling)
        {
            ...
            minX = layer.Map.WorldToTilePositionX(
                cameraClipBounds.Left - (layer.Map.MaxTileWidth * scale.X - tileWidth));
            minY = layer.Map.WorldToTilePositionY(cameraClipBounds.Top -
                                                  (layer.Map.MaxTileHeight * scale.Y - tileHeight));
            maxX = layer.Map.WorldToTilePositionX(cameraClipBounds.Right +
                                                  (layer.Map.MaxTileWidth * scale.X - tileWidth));
    ...
    public static void RenderTile(TmxLayerTile tile, Batcher batcher, Vector2 position, Vector2 scale, float tileWidth,
        float tileHeight, Color color, float layerDepth)
    {
        var gid = tile.Gid;
        ...
        if (tile.Tileset.Image != null)
            batcher.Draw(tile.Tileset.Image.Texture, pos, sourceRect, color, rotation, Vector2.Zero, scale,
                spriteEffects, layerDepth);
        else
            batcher.Draw(tilesetTile.Image.Texture, pos, sourceRect, color, rotation, Vector2.Zero, scale,
                spriteEffects, layerDepth);
    }

    public static void RenderObjectGroup(TmxObjectGroup objGroup, Batcher batcher, Vector2 position, Vector2 scale,
        float layerDepth)
    {
        if (!objGroup.Visible)
            return;
        ...
        foreach (var obj in objGroup.Objects)
        {
            ...
            switch (obj.ObjectType)
            {
                ...
                case TmxObjectType.Tile:
                    ...
                    batcher.Draw(tileset.Image.Texture, pos, sourceRect, Color.White, 0, Vector2.Zero, scale,
                        spriteEffects, layerDepth);
                    goto default;
                ...
                case TmxObjectType.Text:
                    ...
                    batcher.DrawString(Graphics.Instance.BitmapFont, obj.Text.Value, pos, obj.Text.Color,
                        Mathf.Radians(obj.Rotation), Vector2.Zero, fontScale, SpriteEffects.None, layerDepth);
                    goto default;
                default:
                    if (Core.DebugRenderEnabled)
                        batcher.DrawString(Graphics.Instance.BitmapFont, $"{obj.Name} ({obj.Type})",
                            pos - new Vector2(0, 15), Color.Black);
                    break;
            }
        }
    }

    public static void RenderImageLayer(TmxImageLayer layer, Batcher batcher, Vector2 position, Vector2 scale,
        bool repeatX, float layerDepth, RectangleF cameraClipBounds)
    {
        if (!layer.Visible)
            return;
        ...
        var pos = cameraClipBounds.Location -
                  (cameraClipBounds.Location + Screen.Center) *
                  new Vector2(layer.ParallaxFactorX, layer.ParallaxFactorY) +
                  position + new Vector2(layer.OffsetX, layer.OffsetY) * scale + Screen.Center;
        var posRel = pos - cameraClipBounds.Location;

        var minX = layer.RepeatX ? (int)Math.Max(Math.Ceiling(posRel.X / size.X), 0) : 0;
        var minY = layer.RepeatY ? (int)Math.Max(Math.Ceiling(posRel.Y / size.Y), 0) : 0;
        var maxX = layer.RepeatX
            ? (int)Math.Max(Math.Ceiling((cameraClipBounds.Width - posRel.X - size.X) / size.X), 0)
            : 0;
        var maxY = layer.RepeatY
            ? (int)Math.Max(Math.Ceiling((cameraClipBounds.Height - posRel.Y - size.Y) / size.Y), 0)
            : 0;

        for (var x = -minX; x < maxX + 1; x++)
        for (var y = -minY; y < maxY + 1; y++)
            batcher.Draw(layer.Image.Texture, pos + new Vector2(x, y) * size, null, color, 0, Vector2.Zero,
                scale * layer.Scale, x % 2 == 0 ? SpriteEffects.None : SpriteEffects.FlipHorizontally, layerDepth);
    }

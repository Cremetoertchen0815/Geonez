...
namespace Nez.Tiled
{
	public static class TmxLayerTileExt
	{
		...
		{
			int tileSize = self.Tileset.Map.TileWidth;
		...
		public static Edge GetHighestSlopeEdge(this TmxLayerTile self)
		{
			...
			int left = self.GetSlopeTopLeft();
			int right = self.GetSlopeTopRight();
		...
		{
			int tileWidth = self.Tileset.Map.TileWidth;
			int tileMiddleWorldPosition = self.X * tileWidth + tileWidth / 2;

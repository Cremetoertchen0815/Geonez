...
namespace Nez
{
	public static class Input
	{
		...
		public static InputChannel ActiveChannels { get; set; } = InputChannel.Any;
		...
		public static int MaxSupportedGamePads
		{
			...
			set
			{
				...
				for (int i = 0; i < _maxSupportedGamePads; i++)
		...
		public static void Update()
		{
			...
			if (Core.Instance.IsActive && EndlessMouseMode && !IsGameplayDisabled)
			{
				var Center = Screen.Center.ToPoint();
				_previousMouseState = new MouseState(Center.X, Center.Y, _currentMouseState.ScrollWheelValue, _currentMouseState.LeftButton,
													_currentMouseState.MiddleButton, _currentMouseState.RightButton,
			...
			for (int i = 0; i < _virtualInputs.Length; i++)
				_virtualInputs.Buffer[i].Update();
		}


		internal static bool IsInputActive(VirtualInput i) => ActiveChannels == InputChannel.None ? false : (i.Channel == InputChannel.Any || i.Channel == InputChannel.None || ActiveChannels.HasFlag(i.Channel)) & !IsGameplayDisabled;
#if TRACE
		private static bool IsGameplayDisabled => Console.DebugConsole.Instance.IsOpen || Console.DebugConsole.Instance._runtimeInspector != null;
#else
		...
		public static void SetCurrentMouseState(MouseState state) => _currentMouseState = state;
		...
		public static void SetCurrentKeyboardState(KeyboardState state) => _currentKbState = state;
		...
		public static bool IsKeyPressed(Keys key) => _currentKbState.IsKeyDown(key) && !_previousKbState.IsKeyDown(key);
		...
		public static bool IsKeyDown(Keys key) => _currentKbState.IsKeyDown(key);
		...
		public static bool IsKeyReleased(Keys key) => !_currentKbState.IsKeyDown(key) && _previousKbState.IsKeyDown(key);
		...
		/// </summary>
		/// <returns><c>true</c>, if key pressed was gotten, <c>false</c> otherwise.</returns>
		public static bool IsKeyPressed(Keys keyA, Keys keyB) => IsKeyPressed(keyA) || IsKeyPressed(keyB);
		...
		/// </summary>
		/// <returns><c>true</c>, if key down was gotten, <c>false</c> otherwise.</returns>
		public static bool IsKeyDown(Keys keyA, Keys keyB) => IsKeyDown(keyA) || IsKeyDown(keyB);
		...
		/// </summary>
		/// <returns><c>true</c>, if key up was gotten, <c>false</c> otherwise.</returns>
		public static bool IsKeyReleased(Keys keyA, Keys keyB) => IsKeyReleased(keyA) || IsKeyReleased(keyB);
		...
		/// <summary>
		/// If set to true, resets the mouse position to the center of the screen every update cycle.
		/// </summary>
		public static bool EndlessMouseMode { get; set; } = false;
	...
	{
		public bool Equals(InputEventType x, InputEventType y) => x == y;


		public int GetHashCode(InputEventType obj) => (int)obj;
	}
}
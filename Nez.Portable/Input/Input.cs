...
public static class Input
{
    ...
    public static void Update()
    {
        ...
        if (Core.Instance.IsActive && EndlessMouseMode && !IsGameplayDisabled)
        {
            ...
            _previousMouseState = new MouseState(Center.X, Center.Y, _currentMouseState.ScrollWheelValue,
    ...
    internal static bool IsInputActive(VirtualInput i)
    {
        return ActiveChannels == InputChannel.None
            ? false
            : (i.Channel == InputChannel.Any || i.Channel == InputChannel.None || ActiveChannels.HasFlag(i.Channel)) &
              !IsGameplayDisabled;
    }
#if TRACE
    private static bool IsGameplayDisabled =>
        DebugConsole.Instance.IsOpen || DebugConsole.Instance._runtimeInspector != null;
#else
    ...
    /// <summary>
    ///     returns the current mouse state. Use with caution as it only contains raw data and does not take camera scaling
    ///     into affect like
    ///     Input.mousePosition does.
    /// </summary>

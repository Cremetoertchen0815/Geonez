...
namespace Nez
{
	public static class Mathf
	{
		...
		{
			float num = Repeat(target - current, 2 * MathHelper.Pi);
			if (num > MathHelper.Pi)
				num -= 2 * MathHelper.Pi;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static float Map(float value, float leftMin, float leftMax, float rightMin, float rightMax) => rightMin + (value - leftMin) * (rightMax - rightMin) / (leftMax - leftMin);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static Vector2 AngleToVector(float angleRadians, float length) => new Vector2(Cos(angleRadians) * length, Sin(angleRadians) * length);
		...
		{
			float time = Time.TotalTime * speed;
		...
		/// <param name="angleInDegrees">Angle in radians.</param>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static Vector2 PointOnCircleRadians(Vector2 circleCenter, float radius, float angleInRadians) => new Vector2
		{
		...
		{
			float x = Sin(Time.TotalTime * xFrequency + phase) * xMagnitude;
			float y = Cos(Time.TotalTime * yFrequency) * yMagnitude;
		...
		{
			float wrappedTime = PingPong(Time.TotalTime, oscillationInterval);
			float damped = Pow(MathHelper.E, -damping * wrappedTime);

			float x = damped * Sin(Time.TotalTime * xFrequency + phase) * xMagnitude;
			float y = damped * Cos(Time.TotalTime * yFrequency) * yMagnitude;

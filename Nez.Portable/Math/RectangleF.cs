...
namespace Nez
{
	...
	public struct RectangleF : IEquatable<RectangleF>
	{
		...
		public static RectangleF FromMinMax(Vector2 min, Vector2 max) => new RectangleF(min.X, min.Y, max.X - min.X, max.Y - min.Y);
		...
		/// <param name="min">Minimum.</param>
		/// <param name="max">Max.</param>
		public static RectangleF FromMinMax(float minX, float minY, float maxX, float maxY) => new RectangleF(minX, minY, maxX - minX, maxY - minY);
		...
		public void Contains(ref Point value, out bool result) => result = ((((X <= value.X) && (value.X < (X + Width))) && (Y <= value.Y)) &&
					  (value.Y < (Y + Height)));
		...
		public void Contains(ref Vector2 value, out bool result) => result = ((((X <= value.X) && (value.X < (X + Width))) && (Y <= value.Y)) &&
					  (value.Y < (Y + Height)));
		...
		public void Contains(ref RectangleF value, out bool result) => result = ((((X <= value.X) && ((value.X + value.Width) <= (X + Width))) &&
					   (Y <= value.Y)) && ((value.Y + value.Height) <= (Y + Height)));
		...
		public Rectangle GetRectangle() => new Rectangle(Convert.ToInt32(Left), Convert.ToInt32(Top), Convert.ToInt32(Width), Convert.ToInt32(Height));
		...
		public Vector2 GetClosestPointOnRectangleBorderToPoint(Vector2 point, out Vector2 edgeNormal)
		{
			...
			if (Contains(res))
			{
				...
				float min = Mathf.MinOf(dl, dr, dt, db);
				if (min == dt)
				{
					res.Y = Top;
					edgeNormal.Y = -1;
		...
		{
			float x = Math.Max(Math.Max(value1.X, value2.X), 0);
			float y = Math.Max(Math.Max(value1.Y, value2.Y), 0);
			return new RectangleF(x, y,
									...
									float rotation, float width, float height)
		{
			...
			else
			{
				...
				float worldPosX = parentPosition.X + position.X;
				float worldPosY = parentPosition.Y + position.Y;
				...
				float minX = Mathf.MinOf(topLeft.X, bottomRight.X, topRight.X, bottomLeft.X);
				float maxX = Mathf.MaxOf(topLeft.X, bottomRight.X, topRight.X, bottomLeft.X);
				float minY = Mathf.MinOf(topLeft.Y, bottomRight.Y, topRight.Y, bottomLeft.Y);
				float maxY = Mathf.MaxOf(topLeft.Y, bottomRight.Y, topRight.Y, bottomLeft.Y);
		...
		public static implicit operator Rectangle(RectangleF self) => RectangleExt.FromFloats(self.X, self.Y, self.Width, self.Height);

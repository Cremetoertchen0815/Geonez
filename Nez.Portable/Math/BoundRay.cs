...
public struct BoundRay
{
    ...
    public Line ToLine()
    {
        return new Line(Location, Location + Direction * Length);
    }
    ...
    /// <param name="isDistancePercentage">
    ///     If true, interprets the distance as a ratio, with 0 being the start vector and 1
    ///     being the end vector.
    /// </param>
    public Vector2d SlideAlong(double distance, bool isDistancePercentage = false)
    {
        return isDistancePercentage
            ? Location + Direction * Length * Mathf.Clamp01((float)distance)

...
namespace Nez
{
	...
	public struct Matrix2D : IEquatable<Matrix2D>
	{
		...
		public float Rotation
		{
			...
			set
			{
				float val1 = Mathf.Cos(value);
				float val2 = Mathf.Sin(value);
		...
		private static Matrix2D _identity = new Matrix2D(1f, 0f, 0f, 1f, 0f, 0f);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static void CreateScale(float scale, out Matrix2D result) => CreateScale(scale, scale, out result);
		...
		{
			CreateScale(xScale, yScale, out var result);
			return result;
		}
		...
		public static implicit operator Matrix(Matrix2D mat) => new Matrix
			(
				mat.M11, mat.M12, 0, 0,
				mat.M21, mat.M22, 0, 0,
				0, 0, 1, 0,

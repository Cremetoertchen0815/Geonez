...
namespace Nez.Splines
{
	...
	public static class Bezier
	{
		...
		/// <param name="p2">P2.</param>
		/// <param name="t">T.</param>
		public static Vector2 GetFirstDerivative(Vector2 p0, Vector2 p1, Vector2 p2, float t) => 2f * (1f - t) * (p1 - p0) +
				   2f * t * (p2 - p1);
		...
		private static void RecursiveGetOptimizedDrawingPoints(Vector2 start, Vector2 firstCtrlPoint, Vector2 secondCtrlPoint,
													   Vector2 end, List<Vector2> points, float distanceTolerance)
		{
			...
			float d2 = System.Math.Abs(((firstCtrlPoint.X - end.X) * deltaLine.Y -
									  (firstCtrlPoint.Y - end.Y) * deltaLine.X));
			float d3 = System.Math.Abs(((secondCtrlPoint.X - end.X) * deltaLine.Y -

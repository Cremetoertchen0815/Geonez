...
namespace Nez.Tiled
{
...
#endif
	{
		...
		public class CollisionState
		{
			...
			public override string ToString() => string.Format("[CollisionState] r: {0}, l: {1}, a: {2}, b: {3}, angle: {4}, wasGroundedLastFrame: {5}, becameGroundedThisFrame: {6}",
																Right, Left, Above, Below, SlopeAngle, WasGroundedLastFrame, BecameGroundedThisFrame);
		}


		public override void OnAddedToEntity() => _triggerHelper = new ColliderTriggerHelper(Entity);

		private ColliderTriggerHelper _triggerHelper;

		public bool EasyCollision { get; set; } = false;
		...
		/// </summary>
		private List<TmxLayerTile> _collidingTiles = new List<TmxLayerTile>();
		...
		public void TestCollisions(ref Vector2 motion, Rectangle boxColliderBounds, CollisionState collisionState)
		{
			...
			int motionX = (int)motion.X;
			int motionY = (int)motion.Y;
			...
			if (EasyCollision)
			{
				...
				if (direction == Edge.Bottom && TestMapCollision(sweptBounds, direction, collisionState, out int collisionResponse))
				{
			...
			if (motionX != 0)
			{
				...
				if (TestMapCollision(sweptBounds, direction, collisionState, out int collisionResponse))
				{
					// react to collision. get the distance between our leading edge and what we collided with
							  ...
							  out int collisionResponse)
		{
			...
			int perpindicularPosition = side.IsVertical() ? collisionRect.Center.X : collisionRect.Center.Y;
			int leadingPosition = collisionRect.GetSide(direction);
			bool shouldTestSlopes = side.IsVertical();
			PopulateCollidingTiles(collisionRect, direction);

			for (int i = 0; i < _collidingTiles.Count; i++)
			{
				...
				if (collisionState._lastGroundTile != null && direction == Edge.Bottom)
				{
					...
					bool isHighSlopeNearest = _collidingTiles[i].IsSlope() &&
		...
		private bool IsSlopeCollisionRow(int rowY)
		{
			for (int i = 0; i < _collidingTiles.Count; i++)
			{
		...
		private bool TestTileCollision(TmxLayerTile tile, Edge edgeToTest, int perpindicularPosition, int leadingPosition,
							   bool shouldTestSlopes, out int collisionResponse)
		{
				...
				tile.GetNearestEdge(leadingPosition) == tile.GetHighestSlopeEdge())
			{
				...
				bool wasCollidingBeforeMove = moveDir == Edge.Right
			...
			if (shouldTestSlopes)
			{
				int tileWorldX = TiledMap.TileToWorldPositionX(tile.X);
				int tileWorldY = TiledMap.TileToWorldPositionX(tile.Y);
				float slope = tile.GetSlope();
				float offset = tile.GetSlopeOffset();
				...
				bool isColliding = edgeToTest.IsMax()
		...
		private void PopulateCollidingTiles(Rectangle bounds, Edge direction)
		{
			...
			int firstPrimary = WorldToTilePosition(bounds.GetSide(oppositeDirection), primaryAxis);
			int lastPrimary = WorldToTilePosition(bounds.GetSide(direction), primaryAxis);
			int primaryIncr = direction.IsMax() ? 1 : -1;

			int min = WorldToTilePosition(isHorizontal ? bounds.Top : bounds.Left, oppositeAxis);
			int mid = WorldToTilePosition(isHorizontal ? bounds.GetCenter().Y : bounds.GetCenter().X, oppositeAxis);
			int max = WorldToTilePosition(isHorizontal ? bounds.Bottom : bounds.Right, oppositeAxis);

			bool isPositive = mid - min < max - mid;
			int secondaryIncr = isPositive ? 1 : -1;
			int firstSecondary = isPositive ? min : max;
			int lastSecondary = !isPositive ? min : max;

			for (int primary = firstPrimary; primary != lastPrimary + primaryIncr; primary += primaryIncr)
			{
				for (int secondary = firstSecondary;
					secondary != lastSecondary + secondaryIncr;
					secondary += secondaryIncr)
				{
					int col = isHorizontal ? primary : secondary;
					int row = !isHorizontal ? primary : secondary;

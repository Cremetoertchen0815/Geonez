...
namespace Nez
{
	public abstract class Collider : Component
	{
		...
		public override void OnAddedToEntity()
		{
			if (_colliderRequiresAutoSizing)
			{
				...
				if (renderable != null)
				{
					...
					float width = renderableBounds.Width / Entity.Transform.Scale.X;
					float height = renderableBounds.Height / Entity.Transform.Scale.Y;
		...
		public override void OnDisabled() => UnregisterColliderWithPhysicsSystem();
		...
		public bool CollidesWith(Collider collider, Vector2 motion, out CollisionResult result)
		{
			...
			bool didCollide = Shape.CollidesWithShape(collider.Shape, out result);
			if (didCollide)
				result.Collider = collider;

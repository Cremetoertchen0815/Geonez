...
namespace Nez
{
	...
	public class FollowCamera : Component, IUpdatable
	{
		...
		public override void OnRemovedFromEntity() => Core.Emitter.RemoveObserver(CoreEvents.GraphicsDeviceReset, OnGraphicsDeviceReset);
		...
		private void UpdateFollow()
		{
			...
			if (_cameraStyle == CameraStyle.LockOn)
			{
				float targetX = _targetEntity.Transform.Position.X;
				float targetY = _targetEntity.Transform.Position.Y;
		...
		public void Follow(Entity targetEntity, CameraStyle cameraStyle = CameraStyle.CameraWindow)
		{
			_targetEntity = targetEntity;
			_cameraStyle = cameraStyle;
			var cameraBounds = Core.GraphicsDevice.Viewport;

			switch (_cameraStyle)
			{
				case CameraStyle.CameraWindow:
					int w = (cameraBounds.Width / 6);
					int h = (cameraBounds.Height / 3);

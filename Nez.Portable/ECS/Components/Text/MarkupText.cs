...
namespace Nez
{
	...
	public class MarkupText : RenderableComponent
	{
		...
		private Dictionary<string, IFont> _fontDict = new Dictionary<string, IFont>();
		private Dictionary<string, Texture2D> _textureDict = new Dictionary<string, Texture2D>();
		private Dictionary<string, bool> _conditionalDict = new Dictionary<string, bool>();


		public MarkupText() => SetFont("default", Graphics.Instance.BitmapFont);


		public MarkupText(float textWidth) : this() => _textWidth = textWidth;
		...
		public override void Render(Batcher batcher, Camera camera)
		{
			...
			for (int i = 0; i < _compiledMarkup.Count; i++)
			...
			while (reader.Read())
			{
				if (reader.NodeType == XmlNodeType.Element)
				{
					switch (reader.Name)
					{
						...
						case "if":
							{
								...
								bool negateCondition = condition[0] == '!';
								if (negateCondition)
									condition = condition.Substring(1);

								bool value = _conditionalDict[condition];
						...
						case "else":
							{
								bool value = conditionalsStack.Pop();
						...
						case "img":
							{
								...
								float imageWidth = image.Width * scale.X;
						...
						case "nbsp":
							{
								var currentFormatting = formatingStack.Peek();
								float spaceX = currentFormatting.Font.MeasureString(" ").X * currentFormatting.Scale.X;
				...
				else if (reader.NodeType == XmlNodeType.Text)
				{
					...
					var currentFormatting = formatingStack.Peek();
					var re = new Regex(@"\s+");
					string[] words = re.Split(reader.Value);
					float spaceX = currentFormatting.Font.MeasureString(" ").X * currentFormatting.Scale.X;

					int wordIndex = 0;
					float wordRunSize = 0f;
					...
					while (words.Length > wordIndex)
					{
						...
						bool currentWordFits = position.X + wordRunSize + wordSize.X <= _textWidth;

...
namespace Nez.Shadows
{
	...
	public class VisibilityComputer
	{
		...
		private EndPointComparer _radialComparer;
		private static Vector2[] _cornerCache = new Vector2[4];
		private static LinkedList<Segment> _openSegments = new LinkedList<Segment>();


		public VisibilityComputer() => _radialComparer = new EndPointComparer();
		...
		public void AddCircleOccluder(Vector2 position, float radius)
		{
			...
			float angle = Mathf.Atan2(dirToCircle.Y, dirToCircle.X);

			float stepSize = MathHelper.Pi / LineCountForCircleApproximation;
			float startAngle = angle + MathHelper.PiOver2;
			var lastPt = Mathf.AngleToVector(startAngle, radius) + position;
			for (int i = 1; i < LineCountForCircleApproximation; i++)
			{
		...
		/// </summary>        
		public void AddLineOccluder(Vector2 p1, Vector2 p2) => AddSegment(p1, p2);
		...
		private bool IsSegmentInFrontOf(Segment a, Segment b, Vector2 relativeTo)
		{
			...
			bool a1 = IsLeftOf(a.p2.position, a.p1.position, Interpolate(b.p1.position, b.p2.position, 0.01f));
			bool a2 = IsLeftOf(a.p2.position, a.p1.position, Interpolate(b.p2.position, b.p1.position, 0.01f));
			bool a3 = IsLeftOf(a.p2.position, a.p1.position, relativeTo);

			bool b1 = IsLeftOf(b.p2.position, b.p1.position, Interpolate(a.p1.position, a.p2.position, 0.01f));
			bool b2 = IsLeftOf(b.p2.position, b.p1.position, Interpolate(a.p2.position, a.p1.position, 0.01f));
			bool b3 = IsLeftOf(b.p2.position, b.p1.position, relativeTo);
		...
		/// p * (1 - f) + q * f
		/// </summary>        
		private static Vector2 Interpolate(Vector2 p, Vector2 q, float f) => new Vector2(p.X * (1.0f - f) + q.X * f, p.Y * (1.0f - f) + q.Y * f);
	}
}
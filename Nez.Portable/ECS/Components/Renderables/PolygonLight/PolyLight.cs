...
namespace Nez.Shadows
{
	...
	public class PolyLight : RenderableComponent
	{
		...
		private PolygonLightEffect _lightEffect;
		private FastList<short> _indices = new FastList<short>(50);
		private FastList<VertexPositionTexture> _vertices = new FastList<VertexPositionTexture>(20);

		// shared Collider cache used for querying for nearby geometry. Maxes out at 10 Colliders.
		protected static Collider[] _colliderCache = new Collider[10];
		...
		protected virtual int GetOverlappedColliders() => Physics.OverlapCircleAll(Entity.Position + _localOffset, _radius, _colliderCache, CollidesWithLayers);
		...
		private void RenderImpl(Batcher batcher, Camera camera, bool debugDraw)
		{
			...
			{
				int totalOverlaps = GetOverlappedColliders();
				...
				int primitiveCount = _vertices.Length / 2;
				if (primitiveCount == 0)
					return;
		...
		private void GenerateVertsFromEncounters(List<Vector2> encounters)
		{
			...
			int triIndices = _indices.Length / 3;

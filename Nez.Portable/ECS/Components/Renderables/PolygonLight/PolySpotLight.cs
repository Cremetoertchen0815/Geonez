...
namespace Nez.Shadows
{
	...
	public class PolySpotLight : PolyLight
	{
		...
		private void RecalculatePolyPoints()
		{
			...
			float expandedRadius = Radius + Radius * 0.1f;
			int sides = Mathf.CeilToInt(_spotAngle / 25) + 1;
			float stepSize = (_spotAngle * Mathf.Deg2Rad) / sides;
		...
		protected override void LoadVisibilityBoundaries() => _visibility.LoadSpotLightBoundaries(_polygon.Points);

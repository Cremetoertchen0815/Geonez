...
namespace Nez
{
	...
	public class TrailRibbon : RenderableComponent, IUpdatable
	{
		...
		private LinkedList<RibbonSegment> _segments = new LinkedList<RibbonSegment>();
		...
		public TrailRibbon(int ribbonLength) => _ribbonLength = ribbonLength;
		...
		private void InitializeVertices()
		{
			...
			for (int i = 0; i < _ribbonLength; i++)
			{
				float distanceRatio = 1 - (1 / (float)_ribbonLength * (i + 1));
				float segRadius = distanceRatio * RibbonRadius; // the radius size of this current segment
		...
		private void CalculateVertices()
		{
			...
			{
				float ratio = 1 - (1 / (float)_ribbonLength * segCount);

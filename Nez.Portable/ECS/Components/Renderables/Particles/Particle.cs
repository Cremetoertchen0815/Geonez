...
namespace Nez.Particles
{
	...
	public class Particle
	{
		...
		public void Initialize(ParticleEmitterConfig emitterConfig, Vector2 spawnPosition)
		{
			...
			float vectorSpeed = emitterConfig.Speed + emitterConfig.SpeedVariance * Random.MinusOneToOne();
			...
			float startRadius = emitterConfig.MaxRadius + emitterConfig.MaxRadiusVariance * Random.MinusOneToOne();
			float endRadius = emitterConfig.MinRadius + emitterConfig.MinRadiusVariance * Random.MinusOneToOne();
			...
			float particleStartSize = emitterConfig.StartParticleSize +
									emitterConfig.StartParticleSizeVariance * Random.MinusOneToOne();
			float particleFinishSize = emitterConfig.FinishParticleSize +
			...
			float startA = MathHelper.ToRadians(emitterConfig.RotationStart +
											  emitterConfig.RotationStartVariance * Random.MinusOneToOne());
			float endA = MathHelper.ToRadians(emitterConfig.RotationEnd +
						   ...
						   Vector2 rootPosition)
		{
			...
			if (_timeToLive > 0)
			{
				...
				float t = (_particleLifetime - _timeToLive) / _particleLifetime;
				...
				if (collisionConfig.Enabled)
				{
					...
					{
						if (_circleCollisionShape.CollidesWithShape(neighbor.Shape, out var result))
						{
							// handle the overlap
		...
		private void CalculateCollisionResponseVelocity(float friction, float elasticity, ref Vector2 minimumTranslationVector)
		{
			...
			Vector2.Dot(ref _velocity, ref normal, out float n);

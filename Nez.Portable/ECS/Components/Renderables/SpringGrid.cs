...
namespace Nez
{
	...
	public class SpringGrid : RenderableComponent, IUpdatable
	{
		...
		public SpringGrid() : this(new Rectangle(0, 0, Screen.BackbufferWidth, Screen.BackbufferHeight), new Vector2(30))
		{ }

		public SpringGrid(Rectangle gridSize, Vector2 spacing) => SetGridSizeAndSpacing(gridSize, spacing);
		...
		public void SetGridSizeAndSpacing(Rectangle gridSize, Vector2 spacing)
		{
			...
			int numColumns = (int)(gridSize.Width / spacing.X) + 1;
			int numRows = (int)(gridSize.Height / spacing.Y) + 1;
		...
		public void ApplyDirectedForce(Vector2 force, Vector2 position, float radius) => ApplyDirectedForce(new Vector3(force, 0), new Vector3(position, 0), radius);
		...
		{
			_screenSize.X = Screen.BackbufferWidth;
			_screenSize.Y = Screen.BackbufferHeight;

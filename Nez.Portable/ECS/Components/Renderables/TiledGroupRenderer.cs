...
public class TiledGroupRenderer : RenderableComponent
{
    ...
    public void SetGroupToRender(string groupName)
    {
        renderGroup = TiledMap.Groups[groupName];
    }
    ...
    public override void Render(Batcher batcher, Camera camera)
    {
        ...
        for (var i = 0; i < renderGroup.Layers.Count; i++)
            if (renderGroup.Layers[i].Visible)
                TiledRendering.RenderLayer(renderGroup.Layers[i], batcher, Entity.Transform.Position + _localOffset,
                    Transform.Scale, LayerDepth, new RectangleF(camera.Bounds.Center - Screen.Center, Screen.Size));
    }

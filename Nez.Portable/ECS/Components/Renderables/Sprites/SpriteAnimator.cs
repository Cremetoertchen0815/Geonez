...
namespace Nez.Sprites
{
	...
	public class SpriteAnimator : SpriteRenderer, IUpdatable
	{
		...
		/// </summary>
		public Dictionary<string, SpriteAnimation> Animations => _animations;
		...
		public virtual void Update()
		{
			...
			float secondsPerFrame = 1 / (animation.FrameRate * Speed);
			float iterationDuration = secondsPerFrame * animation.Sprites.Length;
			float pingPongIterationDuration = animation.Sprites.Length < 3 ? iterationDuration : secondsPerFrame * (animation.Sprites.Length * 2 - 2);

			_elapsedTime += Time.DeltaTime;
			float time = Math.Abs(_elapsedTime);
		...
		{
			for (int i = 0; i < atlas.AnimationNames.Length; i++)
		...
		public SpriteAnimator AddEverythingFromAtlas(SpriteAtlas atlas)
		{
			...
			foreach (int element in atlas.NonAnimationSprites)
			{
				_animations.Add(atlas.Names[element], new SpriteAnimation(new Sprite[] { atlas.Sprites[element] }, 1));
			}
			return this;
		}
		/// <summary>

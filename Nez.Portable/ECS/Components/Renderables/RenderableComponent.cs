...
namespace Nez
{
	...
	public abstract class RenderableComponent : Component, IRenderable, IComparable<RenderableComponent>
	{
		...
		public RenderableComponent SetRenderLayer(int renderLayer)
		{
			...
			{
				int oldRenderLayer = _renderLayer;
		...
		public T GetMaterial<T>() where T : Material => Material as T;
		...
		{
			int res = other.RenderLayer.CompareTo(RenderLayer);
			if (res == 0)
			{
		...
		public override string ToString() => $"[RenderableComponent] {GetType()}, renderLayer: {RenderLayer}]";

		void IRenderable.Render(Batcher batcher, Camera camera)
		{
#if TRACE
			var timer = DeltaAnalyzer.MeasureSegment(Entity.Name, null, DeltaAnalyzer.DeltaSegmentType.Draw);

...
namespace Nez
{
	...
	public class LineRenderer : RenderableComponent
	{
		...
		private FastList<SegmentPoint> _points = new FastList<SegmentPoint>();
		...
		private Segment _lastSegment = new Segment();
		private FastList<short> _indices = new FastList<short>(50);
		private FastList<VertexPositionColorTexture> _vertices = new FastList<VertexPositionColorTexture>(50);
		...
		private void CalculateVertices()
		{
			...
			float lineLength = 0f;
			float halfMaxWidth = _maxWidth * 0.5f;
			_points.Buffer[0].LengthFromPreviousPoint = 0;
			for (int i = 0; i < _points.Length - 1; i++)
			{
				float distance = Vector2.Distance(_points.Buffer[i].Position, _points.Buffer[i + 1].Position);
			...
			for (int i = 0; i < _points.Length - 1; i++)
			{
				...
				bool hasThirdPoint = _points.Length > i + 2;
				if (hasThirdPoint)
				...
				float firstPointDistance = distanceSoFar;
				distanceSoFar += secondPoint.LengthFromPreviousPoint;

				float firstPointRatio = firstPointDistance / lineLength;
				float secondPointRatio = distanceSoFar / lineLength;
				float thirdPointRatio = 0f;
				if (hasThirdPoint)
		...
		private void PatchStandardJoint(ref Segment segment, ref int vertIndex)
		{
			...
			else
			{
				...
				// all mid segments.
				int firstSegmentOffset = vertIndex == 5 ? 1 : 0;
		...
		private void PatchSmoothJoint(ref Segment segment, ref int vertIndex)
		{
			if (segment.ShouldFuseBottom)
			{
				...
				float angle1 = Mathf.Atan2(a.Y - center.Y, a.X - center.X) * Mathf.Rad2Deg;
				float angle2 = Mathf.Atan2(b.Y - center.Y, b.X - center.X) * Mathf.Rad2Deg;
				float deltaAngle = Mathf.DeltaAngle(angle1, angle2);

				// figure out how many verts we are going to add to the joint
				float totalNewVerts = Mathf.Ceil(System.Math.Abs(deltaAngle) / DegreesPerSubdivision);
				float angleIncrement = deltaAngle / (totalNewVerts + 1);
				...
				for (int i = 0; i < totalNewVerts - 1; i++)
				{
				...
				{
					float midAngle = angle1 + angleIncrement * (i + 1);
		...
		public override void Render(Batcher batcher, Camera camera)
		{
			...
			int primitiveCount = _indices.Length / 3;
		...
		private class Segment
		{
			...
			public void SetPoints(ref SegmentPoint point, ref SegmentPoint nextPoint)
			{
				...
				float radians = Mathf.Atan2(nextPoint.Position.Y - point.Position.Y,

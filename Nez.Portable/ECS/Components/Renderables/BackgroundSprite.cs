...
internal class BackgroundSprite : RenderableComponent
{
    ...
    private static readonly Batcher
        NeutralBatcher = new(Core.GraphicsDevice); //A batcher not influenced by the camera's transformation matrix
    ...
    public override RectangleF Bounds => new(Position.ToVector2(), Size.ToVector2());
    ...
    public override void Render(Batcher batcher, Camera camera)
    {
        ...
        if (LoopHorizontal == LoopMode.ScreenWrap)
            rectangleOriginal.X = (int)Mathf.Repeat(rectangleOriginal.X + Size.X, renderSize.X) - Size.X;
        if (LoopVertical == LoopMode.ScreenWrap)
            rectangleOriginal.Y = (int)Mathf.Repeat(rectangleOriginal.Y + Size.Y, renderSize.Y) - Size.Y;
        ...
        if (LoopHorizontal == LoopMode.FillJump || LoopHorizontal == LoopMode.FillReverse)
        {
            ...
            {
                var fxX = LoopHorizontal == LoopMode.FillReverse && Mathf.IsOdd(x)
                ...
                if (LoopVertical == LoopMode.FillJump || LoopVertical == LoopMode.FillReverse)
                {
                    for (var y = minY; y < maxY; y++)
                    {
                        ...
                        var fxY = LoopVertical == LoopMode.FillReverse && Mathf.IsOdd(y)
                            ? SpriteEffects.FlipVertically
                            : SpriteEffects.None;
                        NeutralBatcher.Draw(Texture,
                            new Rectangle(
                                rectangleOriginal.Location + new Point(x * rectangleOriginal.Width,
                                    y * rectangleOriginal.Height), Size), null, Color.White, fxX | fxY);
                    }
                }
                else
                {
                    ...
                    NeutralBatcher.Draw(Texture,
                        new Rectangle(rectangleOriginal.Location + new Point(x * rectangleOriginal.Width, 0), Size),
                        null, Color.White, fxX);
                }
            }
        }
        else if (LoopVertical == LoopMode.FillJump || LoopVertical == LoopMode.FillReverse)
        {
            ...
            for (var y = minY; y < maxY; y++)
            {
                ...
                var fxY = LoopVertical == LoopMode.FillReverse && Mathf.IsOdd(y)
                    ...
                    : SpriteEffects.None;
                NeutralBatcher.Draw(Texture,
                    new Rectangle(rectangleOriginal.Location + new Point(0, y * rectangleOriginal.Height), Size), null,
                    Color.White, fxY);
            }
        }
        ...
        if (LoopHorizontal == LoopMode.ScreenWrap)
            NeutralBatcher.Draw(Texture,
                new Rectangle(rectangleOriginal.Location + new Point((int)renderSize.X, 0), Size));
        if (LoopVertical == LoopMode.ScreenWrap)
            NeutralBatcher.Draw(Texture,
                new Rectangle(rectangleOriginal.Location + new Point(0, (int)renderSize.Y), Size));
        if (LoopHorizontal == LoopMode.ScreenWrap && LoopVertical == LoopMode.ScreenWrap)
            NeutralBatcher.Draw(Texture, new Rectangle(rectangleOriginal.Location + renderSize.ToPoint(), Size));

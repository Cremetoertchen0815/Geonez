...
namespace Nez
{
	...
	public class SceneManager : GlobalManager, ITelegramReceiver
	{
		...
		public void Rescan(Assembly baseAssembly = null, string rootNamespace = null)
		{
			...
			var allScenes = BaseAssembly.GetTypes().Where(x => x.Namespace != null && x.Namespace.StartsWith(Namespace) & x.IsClass & !x.IsAbstract & x.IsSubclassOf(typeof(Scene))).ToArray();

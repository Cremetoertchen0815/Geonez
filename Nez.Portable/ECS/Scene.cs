...
namespace Nez
{
	public class Scene
	{
		...
		public Point SceneRenderTargetSize =>
			new Point(_sceneRenderTarget?.Bounds.Width ?? Screen.BackbufferWidth, _sceneRenderTarget?.Bounds.Height ?? Screen.BackbufferHeight);
		...
		private RenderTarget2D _sceneRenderTarget { get; set; }
		...
		public virtual void End()
		{
			...
			for (int i = 0; i < _sceneComponents.Length; i++)
		...
		public virtual void Update()
		{
			...
			for (int i = _sceneComponents.Length - 1; i >= 0; i--)
			{
		...
		internal void Render()
		{
			...
			bool lastRendererHadRenderTarget = false;
			for (int i = 0; i < _renderers.Length; i++)
			{
		...
		internal void PostRender(RenderTarget2D finalRenderTarget = null)
		{
			...
			if (EnablePostProcessing)
			{
				for (int i = 0; i < _postProcessors.Length; i++)
				{
					...
					{
						bool isEven = Mathf.IsEven(enabledCounter);
						enabledCounter++;

#if TRACE

						var timer = DeltaAnalyzer.MeasureSegment(_postProcessors.Buffer[i].GetType().Name, null, DeltaAnalyzer.DeltaSegmentType.Draw);
			...
			for (int i = 0; i < _afterPostProcessorRenderers.Length; i++)
			{
				if (i == 0)
				{
			...
			if (_screenshotRequestCallback != null)
			{
				...
				int[] data = new int[tex.Bounds.Width * tex.Bounds.Height];
		...
		private void UpdateResolutionScaler()
		{
			...
			float screenAspectRatio = screenSize.X / (float)screenSize.Y;

			int renderTargetWidth = screenSize.X;
			int renderTargetHeight = screenSize.Y;

			float resolutionScaleX = screenSize.X / (float)designSize.X;
			float resolutionScaleY = screenSize.Y / (float)designSize.Y;

			bool rectCalculated = false;
			...
			switch (_resolutionPolicy)
			{
				...
				case SceneResolutionPolicy.NoBorderPixelPerfect:
					...
					if (designSize.X / (float)designSize.Y < screenAspectRatio)
					{
						float floatScale = screenSize.X / (float)designSize.X;
						PixelPerfectScale = Mathf.CeilToInt(floatScale);
					}
					else
					{
						float floatScale = screenSize.Y / (float)designSize.Y;
						PixelPerfectScale = Mathf.CeilToInt(floatScale);
					}
				...
				case SceneResolutionPolicy.BestFit:
					float safeScaleX = (float)screenSize.X / (designSize.X - _designBleedSize.X);
					float safeScaleY = (float)screenSize.Y / (designSize.Y - _designBleedSize.Y);
			...
			if (!rectCalculated)
			{
				...
				float renderWidth = designSize.X * resolutionScaleX;
				float renderHeight = designSize.Y * resolutionScaleY;
			...
			ScreenTransformMatrix = Matrix.CreateScale(resolutionScaleX, resolutionScaleY, 1F) * Matrix.CreateTranslation(_finalRenderDestinationRect.X, _finalRenderDestinationRect.Y, 0F);


			// set some values in the Input class to translate mouse position to our scaled resolution
			float scaleX = renderTargetWidth / (float)_finalRenderDestinationRect.Width;
			float scaleY = renderTargetHeight / (float)_finalRenderDestinationRect.Height;

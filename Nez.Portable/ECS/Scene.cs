...
public class Scene
{
    ...
    internal FastList<Renderer> _renderers = new();
    ...
    public Point SceneRenderTargetSize =>
        new(_sceneRenderTarget?.Bounds.Width ?? Screen.BackbufferWidth,
            _sceneRenderTarget?.Bounds.Height ?? Screen.BackbufferHeight);
    ...
    public Point GetDesignResolution()
    {
...
#if TRACE
            var timer = DeltaAnalyzer.MeasureSegment(_renderers.Buffer[i].GetType().Name, null,
                DeltaAnalyzer.DeltaSegmentType.Draw);
        ...
        if (EnablePostProcessing)
            for (var i = 0; i < _postProcessors.Length; i++)
                if (_postProcessors.Buffer[i].Enabled)
                {
                    ...
                    var timer = DeltaAnalyzer.MeasureSegment(_postProcessors.Buffer[i].GetType().Name, null,
                        DeltaAnalyzer.DeltaSegmentType.Draw);

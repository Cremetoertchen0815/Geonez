...
namespace Nez
{
	public class Entity : IComparable<Entity>
	{
		...
		protected void CopyFrom(Entity entity)
		{
			...
			for (int i = 0; i < entity.Components._componentsToAdd.Count; i++)
		...
		/// <typeparam name="T">The 1st type parameter.</typeparam>
		public T GetComponent<T>(bool onlyReturnInitializedComponents) where T : Component => Components.GetComponent<T>(onlyReturnInitializedComponents);
		...
		{
			int compare = _updateOrder.CompareTo(other._updateOrder);
			if (compare == 0)
				compare = Id.CompareTo(other.Id);
			return compare;
		}

		public override string ToString() => string.Format("[Entity: name: {0}, tag: {1}, enabled: {2}, depth: {3}]", Name, Tag, Enabled, UpdateOrder);
	}
}
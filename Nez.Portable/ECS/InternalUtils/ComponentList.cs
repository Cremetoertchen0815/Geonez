...
namespace Nez
{
	public class ComponentList
	{
		...
		private static IUpdatableComparer compareUpdatableOrder = new IUpdatableComparer();
		private Entity _entity;

		/// <summary>
		/// list of components added to the entity
		/// </summary>
		private FastList<Component> _components = new FastList<Component>();

		/// <summary>
		/// list of all Components that want update called
		/// </summary>
		private FastList<IUpdatable> _updatableComponents = new FastList<IUpdatable>();
		...
		private List<Component> _tempBufferList = new List<Component>();
		...
		public ComponentList(Entity entity) => _entity = entity;
		...
		public Component[] GetArray() => _components.Buffer;

		#endregion


		public void MarkEntityListUnsorted() => _isComponentListUnsorted = true;

		public FastList<Component> GetBase() => _components;

		public void Add(Component component) => _componentsToAdd.Add(component);
		...
		private void UpdateLists()
		{
			...
			if (_componentsToAdd.Count > 0)
			{
				for (int i = 0, count = _componentsToAdd.Count; i < count; i++)
				{
					...
					component.Entity = _entity; //Prevents Entity from being null, when you immediately remove and readd a component
		...
		{
			UpdateLists();
			for (int i = 0; i < _updatableComponents.Length; i++)
			{
#if TRACE
				var timer = DeltaAnalyzer.MeasureSegment(_entity.Name, _updatableComponents.Buffer[i].GetType().Name, DeltaAnalyzer.DeltaSegmentType.Update);

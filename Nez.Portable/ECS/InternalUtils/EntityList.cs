...
namespace Nez
{
	public class EntityList
	{
		...
		/// </summary>
		private HashSet<Entity> _entitiesToRemove = new HashSet<Entity>();
		...
		/// </summary>
		private Dictionary<int, FastList<Entity>> _entityDict = new Dictionary<int, FastList<Entity>>();
		private HashSet<int> _unsortedTags = new HashSet<int>();

		// used in updateLists to double buffer so that the original lists can be modified elsewhere
		private HashSet<Entity> _tempEntityList = new HashSet<Entity>();


		public EntityList(Scene scene) => Scene = scene;
		...
		public void MarkEntityListUnsorted() => _isEntityListUnsorted = true;

		internal void MarkTagUnsorted(int tag) => _unsortedTags.Add(tag);

		/// <summary>
		/// adds an Entity to the list. All lifecycle methods will be called in the next frame.
		/// </summary>
		/// <param name="entity">Entity.</param>
		public void Add(Entity entity) => _entitiesToAdd.Add(entity);
		...
		/// </summary>
		/// <param name="entity">Entity.</param>
		public bool Contains(Entity entity) => _entities.Contains(entity) || _entitiesToAdd.Contains(entity);
		...
		{
			if (_entityDict.TryGetValue(entity.Tag, out var list))
			{
				list.Remove(entity);
			}
		}
		...
		public void UpdateLists()
		{
			...
			{
				foreach (int tag in _unsortedTags)

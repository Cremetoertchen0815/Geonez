...
{
	/// <summary>
	/// A two-dimensional Camera system with support for 3D matrix generation.
	/// </summary>
	public class Camera : Component
	{
		...
		public Matrix ProjectionMatrix3D
		{
			get
			{
				float targetHeight = (Core.GraphicsDevice.Viewport.Height / _zoom);
				float fov = (float)Math.Atan(targetHeight / (2f * PositionZ3D)) * 2f;
		...
		public Vector2 TransformPosition => -Entity.Transform.Position * _zoom + _origin;
		...
		{
			float newZoom = Mathf.Clamp(zoom, -1, 1);
			if (newZoom == 0)
				_zoom = 1f;
			else if (newZoom < 0)
		...
		/// </summary>
		/// <returns>The to world point.</returns>
		public Vector2 MouseToWorldPoint() => ScreenToWorldPoint(Input.MousePosition);
		...
		/// </summary>
		/// <returns>The to world point.</returns>
		public Vector2 TouchToWorldPoint(TouchLocation touch) => ScreenToWorldPoint(touch.ScaledPosition());

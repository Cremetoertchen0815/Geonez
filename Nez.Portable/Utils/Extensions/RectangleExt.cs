...
namespace Nez
{
	public static class RectangleExt
	{
		...
		public static Rectangle FromFloats(float x, float y, float width, float height) => new Rectangle((int)x, (int)y, (int)width, (int)height);
		...
		public static Rectangle FromMinMaxPoints(Point min, Point max) => new Rectangle(min.X, min.Y, max.X - min.X, max.Y - min.Y);
										   ...
										   Vector2 scale, float rotation, float width, float height)
		{
			...
			else
			{
				...
				Matrix2D.CreateScale(scale.X, scale.Y, out var tempMat); // scale ->
				...
				int minX = (int)Mathf.MinOf(topLeft.X, bottomRight.X, topRight.X, bottomLeft.X);
				int maxX = (int)Mathf.MaxOf(topLeft.X, bottomRight.X, topRight.X, bottomLeft.X);
				int minY = (int)Mathf.MinOf(topLeft.Y, bottomRight.Y, topRight.Y, bottomLeft.Y);
				int maxY = (int)Mathf.MaxOf(topLeft.Y, bottomRight.Y, topRight.Y, bottomLeft.Y);
		...
		public static bool CollisionCheck(ref Rectangle rect, ref Rectangle other, out float moveX, out float moveY)
		{
			...
			int l = other.X - (rect.X + rect.Width);
			int r = (other.X + other.Width) - rect.X;
			int t = other.Y - (rect.Y + rect.Height);
			int b = (other.Y + other.Height) - rect.Y;
		...
		public static Vector2 GetClosestPointOnBoundsToOrigin(ref Rectangle rect)
		{
			...
			int minDist = Math.Abs(rect.Location.X);
		...
		public static Point GetClosestPointOnRectangleBorderToPoint(ref Rectangle rect, Vector2 point)
		{
			...
			{
				int dl = res.X - rect.Left;
				int dr = rect.Right - res.X;
				int dt = res.Y - rect.Top;
				int db = rect.Bottom - res.Y;

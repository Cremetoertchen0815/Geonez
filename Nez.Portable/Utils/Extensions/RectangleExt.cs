...
public static class RectangleExt
{
    ...
    public static bool RayIntersects(ref Rectangle rect, ref Ray2D ray, out float distance)
    {
        ...
        {
            if (ray.Start.Y < rect.Y || ray.Start.Y > rect.Y + rect.Height) return false;
        }
        else
        {
    ...
    public static float? RayIntersects(this Rectangle rectangle, Ray ray)
    {
        ...
        else
        {
            ...
            maxValue = MathHelper.Min(num7, maxValue);
            if (num > maxValue) return null;
        }

        if (Math.Abs(ray.Direction.Y) < 1E-06f)
        {
            if (ray.Position.Y < rectangle.Top || ray.Position.Y > rectangle.Bottom) return null;
        }
        else
        {
            var num10 = 1f / ray.Direction.Y;
    ...
    {
        rect1.Intersects(ref rect2, out var result);
        return result;
    }
    ...
    /// </summary>
    public static Point GetSize(this Rectangle rect)
    {

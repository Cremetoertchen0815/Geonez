...
namespace Nez.Tweens
{
	...
	public static class Easing
	{
		...
		{
			public static float EaseIn(float t, float d) => -(Mathf.Sqrt(1 - (t /= d) * t) - 1);


			public static float EaseOut(float t, float d) => Mathf.Sqrt(1 - (t = t / d - 1) * t);
		...
		{
			public static float EaseIn(float t, float d) => (t == 0) ? 0 : Mathf.Pow(2, 10 * (t / d - 1));


			public static float EaseOut(float t, float d) => t == d ? 1 : (-Mathf.Pow(2, -10 * t / d) + 1);
		...
		{
			public static float EaseIn(float t, float d) => -1 * Mathf.Cos(t / d * (MathHelper.Pi / 2)) + 1f;


			public static float EaseOut(float t, float d) => Mathf.Sin(t / d * (MathHelper.Pi / 2));


			public static float EaseInOut(float t, float d) => -0.5f * (Mathf.Cos(MathHelper.Pi * t / d) - 1);
		}
	}
}
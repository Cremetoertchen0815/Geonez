...
namespace Nez
{
	public static class Screen
	{
		...
		public static Point[] AvailableResolutions { get; set; } = GraphicsAdapter.DefaultAdapter.SupportedDisplayModes.Select(x => new Point(x.Width, x.Height)).GroupBy(x => x).Select(x => x.First()).OrderBy(x => x.Y).OrderBy(x => x.X).ToArray();
		...
		private static Vector2? _customSize = null;
		...
		public static void SetSizeOverride(Vector2? size) => _customSize = size;

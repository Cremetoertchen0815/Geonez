...
public sealed class PriorityQueue<T> : IPriorityQueue<T> where T : PriorityQueueNode
{
    ...
    {
        Insist.IsNotNull(node, "node cannot be null");
        Insist.IsFalse(Count >= _nodes.Length - 1, "Queue is full - node cannot be added: " + node);
    ...
    public T Dequeue()
    {
        Insist.IsFalse(Count <= 0, "Cannot call Dequeue() on an empty queue");
    ...
    public T First
    {
        get
        {
            Insist.IsFalse(Count <= 0, "Cannot call .First on an empty queue");
            return _nodes[1];
        }
    }

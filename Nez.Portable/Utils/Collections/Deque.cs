...
public class Deque<T> : IList<T>
{
    ...
    public int Capacity
    {
        ...
        set
        {
            ...
            if (null != buffer && value == buffer.Length) return;
    ...
    {
        if (Count + numElements > Capacity) Capacity = Count + numElements;
    }
    ...
    public IEnumerator<T> GetEnumerator()
    {
        ...
        {
            for (var i = startOffset; i < Capacity; i++) yield return buffer[i];

            var endIndex = ToBufferIndex(Count);
            for (var i = 0; i < endIndex; i++) yield return buffer[i];
        }
        else
        {
            var endIndex = startOffset + Count;
            for (var i = startOffset; i < endIndex; i++) yield return buffer[i];
        }
    }
    ...
    public void Clear()
    {
        if (Count > 0) ClearBuffer(0, Count);

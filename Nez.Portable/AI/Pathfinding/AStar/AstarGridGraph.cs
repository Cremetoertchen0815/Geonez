...
namespace Nez.AI.Pathfinding
{
	...
	public class AstarGridGraph : IAstarGraph<Point>
	{
		...
		public AstarGridGraph(TmxLayer tiledLayer)
		{
			...
			for (int y = 0; y < tiledLayer.Map.Height; y++)
			{
				for (int x = 0; x < tiledLayer.Map.Width; x++)
				{
		...
		private bool IsNodeInBounds(Point node) => 0 <= node.X && node.X < _width && 0 <= node.Y && node.Y < _height;
		...
		int IAstarGraph<Point>.Cost(Point from, Point to) => WeightedNodes.Contains(to) ? WeightedNodeWeight : DefaultWeight;

		int IAstarGraph<Point>.Heuristic(Point node, Point goal) => Math.Abs(node.X - goal.X) + Math.Abs(node.Y - goal.Y);

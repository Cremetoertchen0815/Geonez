...
namespace Nez.UI
{
	public class Element : ILayout
	{
		...
		public Vector2 ParentToLocalCoordinates(Vector2 parentCoords)
		{
			...
			{
				float cos = Mathf.Cos(MathHelper.ToRadians(rotation));
				float sin = Mathf.Sin(MathHelper.ToRadians(rotation));
		...
		{
			float offsetX = Math.Max(-point.X, point.X - width);
			float offsetY = Math.Max(-point.Y, point.Y - height);

...
namespace Nez.UI
{
	...
	public class Table : Group
	{
		...
		private List<Cell> _cells = new List<Cell>(4);
		private Cell _cellDefaults;
		private List<Cell> _columnDefaults = new List<Cell>(2);
		...
		public Cell Add(string text) => Add(new Label(text));


		/// <summary>
		/// Adds a cell without an element
		/// </summary>
		public Cell Add() => Add((Element)null);
		...
		public override void Layout()
		{
			...
			if (_round)
			{
				...
				{
					var c = _cells[i];
					float elementWidth = Mathf.Round(c.elementWidth);
					float elementHeight = Mathf.Round(c.elementHeight);
					float elementX = Mathf.Round(c.elementX);
					float elementY = Mathf.Round(c.elementY);
		...
		private void Layout(float layoutX, float layoutY, float layoutWidth, float layoutHeight)
		{
			...
			for (int i = 0; i < cellCount; i++)
			{
				...
				float spannedWeightedWidth = 0f;
				int colspan = cell.colspan.Value;
				...
				float prefWidth = cell.prefWidth.Get(cell.element);
				float prefHeight = cell.prefHeight.Get(cell.element);
				float minWidth = cell.minWidth.Get(cell.element);
				float minHeight = cell.minHeight.Get(cell.element);
				float maxWidth = cell.maxWidth.Get(cell.element);
				float maxHeight = cell.maxHeight.Get(cell.element);
		...
		private void ComputeSize()
		{
			...
			for (int i = 0; i < cellCount; i++)
			{
				...
				float spaceRight = cell.spaceRight.Get(cell.element);
				...
				if (colspan == 1)
				{
					...
					float hpadding = cell.computedPadLeft + cell.computedPadRight;
			...
			float hpadding_ = _padLeft.Get(this) + _padRight.Get(this);
			float vpadding_ = _padTop.Get(this) + _padBottom.Get(this);
		...
		private void ComputeDebugRects(float x, float y, float layoutX, float layoutY, float layoutWidth, float layoutHeight,

...
namespace Nez.UI
{
	...
	public class ScrollPane : Group, IInputListener
	{
		...
		public override float PreferredWidth
		{
			get
			{
				...
				{
					float width = ((ILayout)_widget).PreferredWidth;
		...
		void IInputListener.OnMouseEnter() => ResetFade();


		void IInputListener.OnMouseExit() => ResetFade();
		...
		void IInputListener.OnMouseMoved(Vector2 mousePos)
		{
			...
			if (_touchScrollH)
			{
				float delta = mousePos.X - _lastMousePos.X;
				float scrollH = _lastHandlePosition + delta;
				...
				int total = _hScrollBounds.Width - _hKnobBounds.Width;
				if (total != 0)
			...
			else if (_touchScrollV)
			{
				...
				float scrollV = _lastHandlePosition + delta;
				...
				if (total != 0)
				{
					float scrollAmount = (scrollV - _vScrollBounds.Y) / total;
		...
		void IInputListener.OnMouseUp(Vector2 mousePos) => Cancel();


		bool IInputListener.OnMouseScrolled(int mouseWheelDelta)
		{
			ResetFade();
			int scrollDirectionMultiplier = _useNaturalScrolling ? -1 : 1;
			if (_scrollY)
		...
		public Element GetWidget() => _widget;
		...
		/// </summary>
		/// <returns>The scroll x.</returns>
		public float GetScrollX() => _amountX;
		...
		public float GetScrollY() => _amountY;
		...
		public bool GetUseNaturalScrolling() => _useNaturalScrolling;
		...
		public float GetVisualScrollX() => !_scrollX ? 0 : _visualAmountX;


		public float GetVisualScrollY() => !_scrollY ? 0 : _visualAmountY;


		public float GetVisualScrollPercentX() => Mathf.Clamp(_visualAmountX / _maxX, 0, 1);


		public float GetVisualScrollPercentY() => Mathf.Clamp(_visualAmountY / _maxY, 0, 1);


		public float GetScrollPercentX() => Mathf.Clamp(_amountX / _maxX, 0, 1);


		public void SetScrollPercentX(float percentX) => SetScrollX(_maxX * Mathf.Clamp(percentX, 0, 1));


		public float GetScrollPercentY() => Mathf.Clamp(_amountY / _maxY, 0, 1);


		public void SetScrollPercentY(float percentY) => SetScrollY(_maxY * Mathf.Clamp(percentY, 0, 1));
		...
		public float GetScrollWidth() => _areaWidth;
		...
		public float GetScrollHeight() => _areaHeight;
		...
		/// </summary>
		/// <returns>The scroll x.</returns>
		public bool IsScrollX() => _scrollX;
		...
		/// </summary>
		/// <returns>The scroll y.</returns>
		public bool IsScrollY() => _scrollY;
		...
		public bool IsScrollingDisabledX() => _disableX;


		public bool IsScrollingDisabledY() => _disableY;


		public bool IsLeftEdge() => !_scrollX || _amountX <= 0;


		public bool IsRightEdge() => !_scrollX || _amountX >= _maxX;


		public bool IsTopEdge() => !_scrollY || _amountY <= 0;


		public bool IsBottomEdge() => !_scrollY || _amountY >= _maxY;
		...
		public float GetVelocityY() => _velocityY;
		...
		public bool IsForceScrollX() => _forceScrollX;


		public bool IsForceScrollY() => _forceScrollY;
		...
		public bool GetVariableSizeKnobs() => _variableSizeKnobs;
		...
		/// <param name="pixelsX">Pixels x.</param>
		protected void SetVisualScrollX(float pixelsX) => _visualAmountX = pixelsX;
		...
		/// <param name="pixelsY">Pixels y.</param>
		protected void SetVisualScrollY(float pixelsY) => _visualAmountY = pixelsY;
		...
		protected virtual void Update()
		{
			...
			if (_flingTimer > 0)
			{
				...
				float alpha = _flingTimer / _flingTime;
	...
	public class ScrollPaneStyle
	{
		...
		public ScrollPaneStyle Clone() => new ScrollPaneStyle
		{
			Background = Background,
			Corner = Corner,
			HScroll = HScroll,

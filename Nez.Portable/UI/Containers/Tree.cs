...
namespace Nez.UI.Containers
{
	public class Tree : Group, IInputListener
	{
		...
		private readonly List<Node> rootNodes = new List<Node>(0);
		private readonly Selection<Node> selection;
		private float ySpacing = 10, iconSpacingLeft = 2, iconSpacingRight = 2, padding = 0, indentSpacing;
		private float leftColumnWidth, prefWidth, prefHeight;
		private bool sizeInvalid = true;
		private Node foundNode;
		private Node overNode, rangeStart;
		...
		public void Add(Node node) => Insert(rootNodes.Count, node);
		...
		public List<Node> GetNodes() => rootNodes;
		...
		private void Draw(Batcher batch, List<Node> nodes, float indent, Color color)
		{
			...
			for (int i = 0, n = nodes.Count; i < n; i++)
			{
				...
				if (node.icon != null)
				{
					float iconY = actor.GetY() + Mathf.Round((node.height - node.icon.MinHeight) / 2);
				...
				float expandIconY = actor.GetY() + Mathf.Round((node.height - expandIcon.MinHeight) / 2);
		...
		private void SelectNodes(IList<Node> nodes, float low, float high)
		{
			for (int i = 0, n = nodes.Count; i < n; i++)
			{
				var node = nodes[i];
		...
		public Selection<Node> GetSelection() => selection;

		public TreeStyle GetStyle() => style;

		public List<Node> GetRootNodes() => rootNodes;

		public Node GetOverNode() => overNode;
		...
		public void SetOverNode(Node overNode) => this.overNode = overNode;

		public void SetPadding(float padding) => this.padding = padding;

		public float GetIndentSpacing() => indentSpacing;

		public void SetYSpacing(float ySpacing) => this.ySpacing = ySpacing;

		public float GetYSpacing() => ySpacing;
		...
		public void FindExpandedObjects(List<object> objects) => FindExpandedObjects(rootNodes, objects);
		...
		internal static Node FindNode(List<Node> nodes, object o)
		{
			...
			{
				var node = nodes[i];
				var found = FindNode(node.children, o);
				if (found != null)
					return found;
			}
		...
		void IInputListener.OnMouseMoved(Vector2 mousePos) => SetOverNode(GetNodeAt(mousePos.Y));
	...
	public class Node
	{
		...
		public Node(Element element) => actor = element;
		...
		public void Add(Node node) => Insert(children.Count, node);
		...
		public object GetObject() => o;

		public void SetObject(object o) => this.o = o;

		public IDrawable GetIcon() => icon;
		...
		public void SetSelectable(bool selectable) => this.selectable = selectable;

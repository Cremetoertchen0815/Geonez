...
public class Tree : Group, IInputListener
{
    ...
    private float ySpacing = 10, iconSpacingLeft = 2, iconSpacingRight = 2, padding, indentSpacing;
    ...
    private void ComputeSize(List<Node> nodes, float indent)
    {
        ...
        var spacing = iconSpacingLeft + iconSpacingRight;
        for (int i = 0, n = nodes.Count; i < n; i++)
        {
            var node = nodes[i];
            var rowWidth = indent + iconSpacingRight;
    ...
    private void Draw(Batcher batch, List<Node> nodes, float indent, Color color)
    {
        ...
        for (int i = 0, n = nodes.Count; i < n; i++)
        {
            ...
            if (node.expanded) Draw(batch, node.children, indent + indentSpacing, actor.GetColor());
        }
    }

...
namespace Nez.UI
{
	public class TooltipManager
	{
		...
		private List<Tooltip> _shownTooltips = new List<Tooltip>();
		...
		private void StartShowTask(float time) => _showTask = Core.Schedule(time, false, this, t =>
												  {
		...
		private void StartResetTask() => _resetTask = Core.Schedule(ResetTime, false, this, t =>
										 {
											 var tm = t.Context as TooltipManager;
		...
		protected void ShowAction(Tooltip tooltip)
		{
			...
			if (Animations)
			{
				float actionTime = _time > 0 ? 0.3f : 0.15f;

...
namespace Nez.UI
{
	public class Stage
	{
		...
		private bool debugAll, debugUnderMouse, debugParentUnderMouse;
		private Table.TableDebug debugTableUnderMouse = Table.TableDebug.None;
		private Vector2 _lastMousePosition;
		private Element _mouseOverElement;
		private Dictionary<int, Element> _touchOverElement = new Dictionary<int, Element>();
		private List<Element> _inputFocusListeners = new List<Element>();
		private static Keys[] _emptyKeys = new Keys[0];
		private IKeyboardListener _keyboardFocusElement;
		private Keys[] _lastPressedKeys = _emptyKeys;
		...
		/// </summary>
		public T AddElement<T>(T element) where T : Element => root.AddElement(element);
		...
		public Vector2 GetMousePosition() => Entity != null && !IsFullScreen ? Input.ScaledMousePosition : Input.RawMousePosition.ToVector2();
		...
		private void UpdateInputTouch()
		{
			foreach (var touch in Input.Touch.CurrentTouches)
			{
				...
				bool inputPressed = touch.State == Microsoft.Xna.Framework.Input.Touch.TouchLocationState.Pressed;
				bool inputReleased = touch.State == Microsoft.Xna.Framework.Input.Touch.TouchLocationState.Released ||
									touch.State == Microsoft.Xna.Framework.Input.Touch.TouchLocationState.Invalid;
				bool inputMoved = false;
				if (touch.TryGetPreviousLocation(out var prevTouch))
				{
					if (Vector2.Distance(touch.Position, prevTouch.Position) >= float.Epsilon)
						inputMoved = true;
				}

				_touchOverElement.TryGetValue(touch.Id, out var lastOver);
		...
		{
			for (int i = _inputFocusListeners.Count - 1; i >= 0; i--)
			{
		...
		/// </summary>
		public void RemoveInputFocusListener(Element element) => _inputFocusListeners.Remove(element);
		...
		public List<Element> GetElements() => root.children;
		...
		public float GetHeight()
		{
			...
			return Screen.BackbufferHeight;
		}


		public bool GetDebugAll() => debugAll;
		...
		public void SetDebugTableUnderMouse(bool debugTableUnderMouse) => SetDebugTableUnderMouse(debugTableUnderMouse ? Table.TableDebug.All : Table.TableDebug.None);


		/// <summary>
		/// Removes the touch, keyboard, and scroll focused elements
		/// </summary>
		public void UnfocusAll() => SetKeyboardFocus(null);
		...
		public IKeyboardListener GetKeyboardFocus() => _keyboardFocusElement;
		...
		private IGamepadFocusable FindNextGamepadFocusable(IGamepadFocusable relativeToFocusable, Direction direction)
		{
			...
			float distanceToNextButton = float.MaxValue;
			...
			for (int i = 0; i < buttons.Count; i++)
			{
				...
				if (isDirectionMatch)
				{
					...
					{
						float distance = Vector2.DistanceSquared(currentCoords, buttonCoords);

...
public class Stage
{
    ...
    public void Update()
    {
        ...
        if (Input.Touch.IsConnected && Input.Touch.CurrentTouches.Count > 0) UpdateInputTouch();
    }
        ...
        ref Element lastOver)
    {
        ...
        if (inputPressed) UpdateInputDown(inputPos, over);

        if (inputMoved) UpdateInputMoved(inputPos, over, lastOver);

        if (inputReleased) UpdateInputReleased(inputPos);

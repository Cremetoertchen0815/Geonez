...
public class Skin
{
    ...
    {
        if (!_resources.TryGetValue(type, out var typedResources))
        {
            typedResources = new Dictionary<string, object>();
    ...
    public NinePatchSprite GetNinePatchSprite(string name)
    {
    ...
    /// <summary>
    ///     Returns a registered tiled drawable. If no tiled drawable is found but a Sprite exists with the name, a tiled
    ///     drawable is
    ...
    /// <summary>
    ///     Returns a registered ninepatch. If no ninepatch is found but a Sprite exists with the name, a ninepatch is created
    ///     from the

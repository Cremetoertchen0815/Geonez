...
namespace Nez.UI
{
	public class Skin
	{
		...
		public T SetStylesForStyleClass<T>(T styleClass, Dictionary<string, object> styleDict, NezContentManager contentManager, string styleName)
		{
			...
			{
				string name = styleConfig.Key;
				object valueObject = styleConfig.Value;
				string identifier = valueObject.ToString();
				...
				else if (name.ToLower().EndsWith("style"))
				{
					...
					if (valueObject is Dictionary<string, object>)
					{
						...
						object inlineStyle = Activator.CreateInstance(styleField.FieldType);
					...
					else
					{
						...
						object theStyle = getStyleMethod.Invoke(this, new object[] { identifier });

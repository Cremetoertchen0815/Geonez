...
namespace Nez.UI
{
	public class ProgressBar : Element
	{
		...
		protected virtual IDrawable GetKnobDrawable() => (Disabled && style.DisabledKnob != null) ? style.DisabledKnob : style.Knob;
		...
		public override void Draw(Batcher batcher, float parentAlpha)
		{
			...
			if (_vertical)
			{
				...
				if (Min != Max)
				{
					...
					{
						float bgBottomHeight = bg != null ? bg.BottomHeight : 0;
		...
		public float GetVisualPercent() => (_value - Min) / (Max - Min);
	...
	public class ProgressBarStyle
	{
		...
		public ProgressBarStyle Clone() => new ProgressBarStyle
		{
			Background = Background,

...
namespace Nez
{
	...
	public class IMGUI
	{
		...
		private static Batcher _batcher;
		...
		private static Vector2 FONT_SCALE;

		// colors
		private static Color FONT_COLOR = new Color(255, 255, 255);
		private static Color WINDOW_COLOR = new Color(17, 17, 17);
		private static Color BUTTON_COLOR = new Color(78, 91, 98);
		private static Color BUTTON_COLOR_ACTIVE = new Color(168, 207, 115);
		private static Color BUTTON_COLOR_DOWN = new Color(244, 23, 135);
		private static Color TOGGLE_BG = new Color(63, 63, 63);
		private static Color TOGGLE_BG_ACTIVE = new Color(130, 130, 130);
		private static Color TOGGLE_ON = new Color(168, 207, 115);
		private static Color TOGGLE_ON_ACTIVE = new Color(244, 23, 135);
		private static Color SLIDER_BG = new Color(78, 91, 98);
		private static Color SLIDER_THUMB_BG = new Color(25, 144, 188);
		private static Color SLIDER_THUMB_BG_ACTIVE = new Color(168, 207, 115);
		private static Color SLIDER_THUMB_BG_DOWN = new Color(244, 23, 135);
		private static Color HEADER_BG = new Color(40, 46, 50);
...
#pragma warning disable 0414
		...
		static IMGUI()
		{
			...
			float scale = FONT_LINE_HEIGHT / _font.LineHeight;
							   ...
							   float elementHeight = ELEMENT_HEIGHT)
		{
			...
			float y = _lastY + ELEMENT_PADDING + (elementHeight - FONT_LINE_HEIGHT) * 0.5f;
		...
		private static void EndElement(float elementHeight = ELEMENT_HEIGHT) => _lastY += elementHeight + ELEMENT_PADDING;
		...
		public static void EndWindow() => _batcher.End();
		...
		{
			float toggleX = _elementX + _elementWidth - ELEMENT_HEIGHT;
			var color = TOGGLE_BG;
		...
		{
			float workingWidth = _elementWidth - SHORT_ELEMENT_HEIGHT;
			float thumbPos = workingWidth * value;
			...
			if (IsMouseOverElement())
			{
				if (Input.LeftMouseButtonDown)
				{
					float localMouseX = _mouseInWorldCoords.X - _elementX - SHORT_ELEMENT_HEIGHT * 0.5f;
		...
		{
			float thumbPos = _elementWidth * value;
			var color = SLIDER_THUMB_BG;
			...
			if (IsMouseOverElement())
			{
				if (Input.LeftMouseButtonDown)
				{
					float localMouseX = _mouseInWorldCoords.X - _elementX;
		...
		public static void Space(float verticalSpace) => _lastY += verticalSpace;
	}
}
...
namespace Nez.DeferredLighting
{
	...
	public class DeferredLightingRenderer : Renderer
	{
		...
		private DeferredLightEffect _lightEffect;

		// light volumes. quad for directional/area and polygon for others
		private QuadMesh _quadMesh;
		private PolygonMesh _polygonMesh;
		private PolygonMesh _quadPolygonMesh;
		...
		protected override void DebugRender(Scene scene, Camera cam)
		{
			...
			for (int j = 0; j < lightRenderables.Length; j++)
			{
		...
		private void RenderAllBuffers(Scene scene)
		{
			...
			int halfWidth = tempRT.Width / 2;
			int halfHeight = tempRT.Height / 2;

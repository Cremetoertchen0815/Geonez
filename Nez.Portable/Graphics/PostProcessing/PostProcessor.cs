...
namespace Nez
{
	...
	public class PostProcessor : IComparable<PostProcessor>
	{
		...
		public virtual void OnAddedToScene(Scene scene) => _scene = scene;
		...
		public virtual void Process(RenderTarget2D source, RenderTarget2D destination) => DrawFullscreenQuad(source, destination, Effect);
		...
		public int CompareTo(PostProcessor other) => ExecutionOrder.CompareTo(other.ExecutionOrder);
	}
}
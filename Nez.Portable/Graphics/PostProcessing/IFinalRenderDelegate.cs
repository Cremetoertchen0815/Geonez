...
///     the Scene.screenshotRequestCallback will not function as expected if an IFinalRenderDelegate is present. This is
///     because the
...
public interface IFinalRenderDelegate
{
    ...
    /// <summary>
    ///     this gets called by a Scene so that the final render can be handled. The render should be done into
    ///     finalRenderTarget.
    ///     In most cases, finalRenderTarget will be null so the render will just be to the backbuffer. The only time
    ///     finalRenderTarget

...
namespace Nez
{
	...
	public class StencilLightRenderer : Renderer
	{
		...
		private const int CIRCLE_APPROXIMATION_VERTS = 12;
		private Vector2[] _vertBuffer = new Vector2[4];
		private PrimitiveBatch _primitiveBatch;
		private DepthStencilState _depthStencilState;
		private BlendState _stencilOnlyBlendState;
		...
		private void RenderLight(IRenderable renderable, Camera cam)
		{
			// fetch all the Colliders in range
			var colliders = Physics.BoxcastBroadphase(renderable.Bounds, CollidesWithLayers);
			int colliderCount = IEnumerableExtensions.IEnumerableExt.Count(colliders);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void RenderPolygon(Polygon polygon, Vector2 lightPos) => RenderVerts(polygon.position, lightPos, polygon.Points);

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void RenderCircle(Circle circle, Vector2 lightPos)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void RenderVerts(Vector2 position, Vector2 lightPos, Vector2[] verts)
		{
			...
			for (int i = 0; i < verts.Length; i++)
			{
				...
				Vector2.Dot(ref normal, ref lightToStart, out float nDotL);
				if (nDotL > 0)
				{
					...
					var point1 = nextVertex + (Vector2.Normalize(nextVertex - lightPos) * Screen.BackbufferWidth);

...
public static class BatcherDrawingExt
{
    ...
    public static void DrawLine(this Batcher batcher, Vector2 start, Vector2 end, Color color, float thickness,
        float extension = 0)
    {
        DrawLineAngle(batcher, start - Vector2.Normalize(end - start) * extension,
            Mathf.AngleBetweenVectors(start, end), Vector2.Distance(start, end) + extension * 2, color,
            thickness);
    }

    public static void DrawTexLine(this Batcher batcher, Sprite sprite, Vector2 start, Vector2 end, Color color,
        float thickness)
    {
        DrawTexLineAngle(batcher, sprite, start, Mathf.AngleBetweenVectors(start, end), Vector2.Distance(start, end),
            color,
            thickness);
    }
    ...
    public static void DrawTexLineAngle(this Batcher batcher, Sprite texture, Vector2 start, float radians,
        float length, Color color,
        float thickness)
    {
        batcher.Draw(texture, start, Graphics.Instance.PixelTexture.SourceRect, color,
            radians, new Vector2(0f, 0.5f),
            new Vector2(length / texture.Texture2D.Width, thickness / texture.Texture2D.Height), SpriteEffects.None, 0);
    }

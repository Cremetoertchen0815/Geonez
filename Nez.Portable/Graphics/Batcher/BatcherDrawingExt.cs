...
namespace Nez
{
	...
	public static class BatcherDrawingExt
	{
		...
		public static void DrawLine(this Batcher batcher, Vector2 start, Vector2 end, Color color) => DrawLineAngle(batcher, start, Mathf.AngleBetweenVectors(start, end), Vector2.Distance(start, end), color);


		public static void DrawLine(this Batcher batcher, Vector2 start, Vector2 end, Color color, float thickness, float extension = 0) => DrawLineAngle(batcher, start - Vector2.Normalize(end - start) * extension, Mathf.AngleBetweenVectors(start, end), Vector2.Distance(start, end) + extension * 2, color,
				thickness);
		public static void DrawTexLine(this Batcher batcher, Textures.Sprite sprite, Vector2 start, Vector2 end, Color color, float thickness) => DrawTexLineAngle(batcher, sprite, start, Mathf.AngleBetweenVectors(start, end), Vector2.Distance(start, end), color,
				thickness);
		...
		public static void DrawLineAngle(this Batcher batcher, Vector2 start, float radians, float length, Color color) => batcher.Draw(Graphics.Instance.PixelTexture, start, Graphics.Instance.PixelTexture.SourceRect, color,
		...
		public static void DrawLineAngle(this Batcher batcher, Vector2 start, float radians, float length, Color color,
										 float thickness) => batcher.Draw(Graphics.Instance.PixelTexture, start, Graphics.Instance.PixelTexture.SourceRect, color,
				radians, new Vector2(0f, 0.5f), new Vector2(length, thickness), SpriteEffects.None, 0);

		public static void DrawTexLineAngle(this Batcher batcher, Textures.Sprite texture, Vector2 start, float radians, float length, Color color,
										 float thickness) => batcher.Draw(texture, start, Graphics.Instance.PixelTexture.SourceRect, color,
				radians, new Vector2(0f, 0.5f), new Vector2(length / texture.Texture2D.Width, thickness / texture.Texture2D.Height), SpriteEffects.None, 0);


		public static void DrawLineAngle(this Batcher batcher, float startX, float startY, float radians, float length,
										 Color color) => DrawLineAngle(batcher, new Vector2(startX, startY), radians, length, color);
		...
		public static void DrawCircle(this Batcher batcher, float x, float y, float radius, Color color,
									  int thickness = 1, int resolution = 12) => DrawCircle(batcher, new Vector2(x, y), radius, color, thickness, resolution);
		...
		public static void DrawHollowRect(this Batcher batcher, Vector2 position, float width, float height,
										  Color color, float thickness = 1) => DrawHollowRect(batcher, position.X, position.Y, width, height, color, thickness);


		public static void DrawHollowRect(this Batcher batcher, Rectangle rect, Color color, float thickness = 1) => DrawHollowRect(batcher, rect.X, rect.Y, rect.Width, rect.Height, color, thickness);

...
namespace Nez
{
	public class Batcher : GraphicsResource
	{
		...
		public void SetIgnoreRoundingDestinations(bool shouldIgnore) => _shouldIgnoreRoundingDestinations = shouldIgnore;

		#region Public begin/end methods

		public void Begin() => Begin(BlendState.AlphaBlend, Core.DefaultSamplerState, DepthStencilState.None,
				RasterizerState.CullCounterClockwise, null, Matrix.Identity, false);

		public void Begin(Effect effect) => Begin(BlendState.AlphaBlend, Core.DefaultSamplerState, DepthStencilState.None,
				RasterizerState.CullCounterClockwise, effect, Matrix.Identity, false);

		public void Begin(Material material) => Begin(material.BlendState, material.SamplerState, material.DepthStencilState,
				RasterizerState.CullCounterClockwise, material.Effect);

		public void Begin(Matrix transformationMatrix) => Begin(BlendState.AlphaBlend, Core.DefaultSamplerState, DepthStencilState.None,
				RasterizerState.CullCounterClockwise, null, transformationMatrix, false);

		public void Begin(BlendState blendState) => Begin(blendState, Core.DefaultSamplerState, DepthStencilState.None, RasterizerState.CullCounterClockwise,
				null, Matrix.Identity, false);

		public void Begin(Material material, Matrix transformationMatrix) => Begin(material.BlendState, material.SamplerState, material.DepthStencilState,
		...
		public void Begin(BlendState blendState, SamplerState samplerState, DepthStencilState depthStencilState,
						  RasterizerState rasterizerState) => Begin(
				blendState,
				samplerState,
				depthStencilState,
				rasterizerState,
		...
		public void Begin(BlendState blendState, SamplerState samplerState, DepthStencilState depthStencilState,
						  RasterizerState rasterizerState, Effect effect) => Begin(
				blendState,
				samplerState,
				depthStencilState,
				rasterizerState,
		...
		public void Begin(BlendState blendState, SamplerState samplerState, DepthStencilState depthStencilState,
						  ...
						  Effect effect, Matrix transformationMatrix) => Begin(
				blendState,
				samplerState,
				depthStencilState,
				rasterizerState,
		...
		public void DrawString(SpriteFont spriteFont, string text, Vector2 position, Color color, float rotation,
							   Vector2 origin, Vector2 scale, SpriteEffects effects, float layerDepth) => throw new NotImplementedException("SpriteFont is too locked down to use directly. Wrap it in a NezSpriteFont");
		...
		private unsafe void PushSprite(Texture2D texture, Rectangle? sourceRectangle, float destinationX, float destinationY,
						...
						float skewBottomX, float skewLeftY, float skewRightY)
		{
			...
			{
				float inverseTexW = 1.0f / texture.Width;
				float inverseTexH = 1.0f / texture.Height;
			...
			fixed (VertexPositionColorTexture4* vertexInfo = &_vertexInfo[_numSprites])
			{
				// calculate vertices
				// top-left
				float cornerX = (_cornerOffsetX[0] - originX) * destinationW + skewTopX;
				float cornerY = (_cornerOffsetY[0] - originY) * destinationH - skewLeftY;
		...
		/// </summary>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private unsafe void PushSprite(Sprite sprite, float destinationX, float destinationY, float destinationW,
						...
						float skewRightY)
		{
			...
			float originX = (origin.X / sprite.Uvs.Width) / sprite.Texture2D.Width;
			float originY = (origin.Y / sprite.Uvs.Height) / sprite.Texture2D.Height;
		...
		{
			bool currentValue = _rasterizerState.ScissorTestEnable;
		...
		private void PrepRenderState()
		{
			...
			{
				_projectionMatrix.M11 = (float)(2.0 / (viewport.Width / 2 * 2 - 1));
				_projectionMatrix.M22 = (float)(-2.0 / (viewport.Height / 2 * 2 - 1));
			}
			else
			{

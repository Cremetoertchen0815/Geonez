...
namespace Nez.Svg
{
	...
	public class SvgImage : SvgElement
	{
		...
		public Texture2D GetTexture(NezContentManager content)
		{
			...
			{
				int startIndex = Href.IndexOf("base64,", StringComparison.OrdinalIgnoreCase) + 7;
				string imageContents = Href.Substring(startIndex);
				byte[] bytes = Convert.FromBase64String(imageContents);

...
namespace Nez.Svg
{
	public static class SvgPathParser
	{
		...
		public static List<SvgPathSegment> Parse(string path)
		{
			...
			try
			{
				...
				foreach (string commandSet in SplitCommands(path.TrimEnd(null)))
				{
					...
					string remainingargs = commandSet.Substring(1);
		...
		private static Vector2 ToAbsolute(float x, float y, List<SvgPathSegment> segments, bool areBothRelative) => ToAbsolute(x, y, segments, areBothRelative, areBothRelative);
		...
		private static Vector2 ToAbsolute(float x, float y, List<SvgPathSegment> segments, bool isRelativeX, bool isRelativeY)
		{

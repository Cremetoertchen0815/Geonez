...
namespace Nez.Textures
{
	...
	public class Sprite
	{
		...
		public Sprite(Texture2D texture, Rectangle sourceRect, Vector2 origin)
		{
			...
			float inverseTexW = 1.0f / Texture2D.Width;
			float inverseTexH = 1.0f / Texture2D.Height;
										   ...
										   int marginTop, int marginBottom)
		{
			...
			int stretchedCenterWidth = renderRect.Width - marginLeft - marginRight;
			int stretchedCenterHeight = renderRect.Height - marginTop - marginBottom;
			int bottomY = renderRect.Y + renderRect.Height - marginBottom;
			int rightX = renderRect.X + renderRect.Width - marginRight;
			int leftX = renderRect.X + marginLeft;
			int topY = renderRect.Y + marginTop;
		...
		/// clones the Sprite
		/// </summary>
		public Sprite Clone() => new Sprite(Texture2D, SourceRect)
		{
			Origin = Origin
		};
		...
		public static Texture2D GenerateStencil(Texture2D input)
		{
			//Extract pixel data from source texture
			var res = new Texture2D(input.GraphicsDevice, input.Width, input.Height, false, input.Format);
			var data = new Color[res.Width * res.Height];
			input.GetData<Color>(data);

			//Manipulate pixels
			for (int i = 0; i < data.Length; i++)
				data[i] = data[i].A > 50 ? Color.White : Color.Transparent;

			//Write data into new texture

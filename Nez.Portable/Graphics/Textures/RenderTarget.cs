...
namespace Nez.Textures
{
	public class RenderTarget : GlobalManager
	{
		...
		private List<TrackedRenderTarget2D> _renderTargetPool = new List<TrackedRenderTarget2D>();


		public RenderTarget() => instance = this;
		...
		public static RenderTarget2D GetTemporary(int width, int height) => GetTemporary(width, height, Screen.PreferredDepthStencilFormat);
		...
		public static RenderTarget2D GetTemporary(int width, int height, DepthFormat depthFormat)
		{
			...
			for (int i = 0; i < instance._renderTargetPool.Count; i++)
			{
		...
		public override void Update()
		{
			...
			for (int i = _renderTargetPool.Count - 1; i >= 0; i--)
			{

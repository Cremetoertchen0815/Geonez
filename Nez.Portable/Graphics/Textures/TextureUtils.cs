...
namespace Nez.Textures
{
	...
	public static class TextureUtils
	{
		...
		private static unsafe void PremultiplyAlpha(byte[] pixels)
		{
		...
		public static Texture2D CreateBlurredGrayscaleTexture(Texture2D image, double deviation = 1) => GaussianBlur.CreateBlurredGrayscaleTexture(image, deviation);

		public static Color[] CreateBlurredTexture(Color[] srcData, int width, int height, double deviation = 1) => GaussianBlur.CreateBlurredTexture(srcData, width, height, deviation);
		...
		/// <param name="image">Image.</param>
		/// <param name="deviation">Deviation.</param>
		public static Texture2D CreateBlurredTexture(Texture2D image, double deviation = 1) => GaussianBlur.CreateBlurredTexture(image, deviation);

		public static Color[] CreateBlurredGrayscaleTexture(Color[] srcData, int width, int height,
															double deviation = 1) => GaussianBlur.CreateBlurredGrayscaleTexture(srcData, width, height, deviation);
											  ...
											  float normalStrength = 1f, bool invertX = false, bool invertY = false)
		{
			...
			float invertR = invertX ? -1f : 1f;
			float invertG = invertY ? -1f : 1f;
			...
			for (int i = 1; i < width - 1; i++)
			{
				...
				{
					float c = srcData[i + j * width].Grayscale().B / 255f;
					float r = srcData[i + 1 + j * width].Grayscale().B / 255f;
					float l = srcData[i - 1 + j * width].Grayscale().B / 255f;
					float t = srcData[i + (j - 1) * width].Grayscale().B / 255f;
					float b = srcData[i + (j + 1) * width].Grayscale().B / 255f;
					float bl = srcData[i - 1 + (j + 1) * width].Grayscale().B / 255f;
					float tl = srcData[i - 1 + (j - 1) * width].Grayscale().B / 255f;
					float br = srcData[i + 1 + (j + 1) * width].Grayscale().B / 255f;
					float tr = srcData[i + 1 + (j - 1) * width].Grayscale().B / 255f;

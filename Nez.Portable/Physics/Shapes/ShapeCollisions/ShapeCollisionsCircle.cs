...
namespace Nez.PhysicsShapes
{
	public static partial class ShapeCollisions
	{
		public static bool CircleToCircleCast(Circle first, Circle second, Vector2 deltaMovement, out RaycastHit hit)
		{
			...
			float closestDistanceSquared = Vector2.DistanceSquared(second.position, d);
			float sumOfRadiiSquared = (first.Radius + second.Radius) * (first.Radius + second.Radius);
			...
			if (closestDistanceSquared <= sumOfRadiiSquared)
			{
				...
				float backDist = Mathf.Sqrt(sumOfRadiiSquared - closestDistanceSquared);
		...
		public static bool CircleToCircle(Circle first, Circle second, out CollisionResult result)
		{
			...
			float distanceSquared = Vector2.DistanceSquared(first.position, second.position);
			float sumOfRadii = first.Radius + second.Radius;
		...
		public static bool CircleToBox(Circle circle, Box box, out CollisionResult result)
		{
			...
			Vector2.DistanceSquared(ref closestPointOnBounds, ref circle.position, out float sqrDistance);
			...
			else if (sqrDistance <= circle.Radius * circle.Radius)
			{
				...
				float depth = result.Normal.Length() - circle.Radius;
		...
		public static bool CircleToPolygon(Circle circle, Polygon polygon, out CollisionResult result)
		{
			...
			bool circleCenterInsidePoly = polygon.ContainsPoint(circle.position);
		...
		private static bool CircleToPolygon2(Circle circle, Polygon polygon, out CollisionResult result)
		{
			...
			{
				float projection = Vector2.Dot(polygon.Points[i], poly2CircleNormalized);
				if (max < projection)
				{
					closestPointIndex = i;
					max = projection;
			...
			float poly2CircleLength = poly2Circle.Length();
			...
			int prePointIndex = closestPointIndex - 1;
			int postPointIndex = closestPointIndex + 1;
			...
			var closest2 = ClosestPointOnLine(polygon.Points[closestPointIndex], polygon.Points[postPointIndex],
				circleCenter);
			Vector2.DistanceSquared(ref circleCenter, ref closest1, out float distance1);
			Vector2.DistanceSquared(ref circleCenter, ref closest2, out float distance2);

...
public static partial class ShapeCollisions
{
	...
	public static bool BoxToBoxCast(Box first, Box second, Vector2 movement, out RaycastHit hit)
    {
        ...
        if (minkowskiDiff.RayIntersects(ref ray, out var fraction) && fraction <= 1.0f)
        {
        ...
        out RaycastHit hit)
    {
        ...
        if (PolygonToPolygon(first, second, deltaMovement, out hit.Normal, out var timeOfCollision))
        {
    ...
    public static bool PolygonToPolygonOLD(Polygon first, Polygon second, out CollisionResult result)
    {
        ...
        if (PolygonToPolygon(first, second, null, out result.Normal, out var timeOfCollision))
        {
        ...
        ref Vector2? deltaMovement, out float taxis)
    {
        ...
        GetInterval(first, first.Points.Length, axis, out var min0, out var max0);
        GetInterval(second, second.Points.Length, axis, out var min1, out var max1);

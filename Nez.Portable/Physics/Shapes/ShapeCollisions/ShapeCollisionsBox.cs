	...
	public static partial class ShapeCollisions
	{
		...
		public static bool BoxToBoxCast(Box first, Box second, Vector2 movement, out RaycastHit hit)
		{
			...
			else
			{
				...
				if (minkowskiDiff.RayIntersects(ref ray, out float fraction) && fraction <= 1.0f)
				{
		...
		private static Vector2[] _satAxisArray = new Vector2[0];
		private static float[] _satTimerPerAxis = new float[0];
												...
												out RaycastHit hit)
		{
			...
			if (PolygonToPolygon(first, second, deltaMovement, out hit.Normal, out float timeOfCollision))
			{
		...
		public static bool PolygonToPolygonOLD(Polygon first, Polygon second, out CollisionResult result)
		{
			...
			if (PolygonToPolygon(first, second, null, out result.Normal, out float timeOfCollision))
			{
		...
		[Obsolete]
		private static bool PolygonToPolygon(Polygon first, Polygon second, Vector2? deltaMovement, out Vector2 responseNormal,
									 out float timeOfCollision)
		{
			...
			if (deltaMovement.HasValue)
			{
				...
				float fVel2 = Vector2.Dot(deltaMovement.Value, deltaMovement.Value);
									  ...
									  ref Vector2? deltaMovement, out float taxis)
		{
			...
			GetInterval(first, first.Points.Length, axis, out float min0, out float max0);
			GetInterval(second, second.Points.Length, axis, out float min1, out float max1);

			float h = Vector2.Dot(shapeOffset, axis);
			min0 += h;
			max0 += h;
			...
			if (d0 > 0.0f || d1 > 0.0f)
			{
				...
				float v = Vector2.Dot(deltaMovement.Value, axis);
		...
		private static void GetInterval(Polygon polygon, int numVertices, Vector2 axis, out float min, out float max)
		{
			...
			{
				float d = Vector2.Dot(polygon.Points[i], axis);
				if (d < min)
					min = d;
				else if (d > max)
					max = d;
		...
		[Obsolete]
		private static bool FindMinimumTranslationDistance(int iNumAxes, out Vector2 normal, out float timeOfIntersection)
		{
			...
			for (int i = 0; i < iNumAxes; i++)
			{
				float n = _satAxisArray[i].Length();

...
namespace Nez.PhysicsShapes
{
	public class Polygon : Shape
	{
		...
		internal Polygon(Vector2[] points, bool isBox) : this(points) => this.isBox = isBox;
		...
		private void BuildEdgeNormals()
		{
			...
			int totalEdges = isBox ? 2 : Points.Length;
		...
		public static Vector2[] BuildSymmetricalPolygon(int vertCount, float radius)
		{
			...
			{
				float a = 2.0f * MathHelper.Pi * (i / (float)vertCount);
		...
		internal override void RecalculateBounds(Collider collider)
		{
			...
			if (collider.ShouldColliderScaleAndRotateWithTransform)
			{
				...
				if (collider.Entity.Transform.Rotation != 0)
				{
					...
					float offsetAngle = Mathf.Atan2(collider.LocalOffset.Y * collider.Entity.Transform.Scale.Y, collider.LocalOffset.X * collider.Entity.Transform.Scale.X) * Mathf.Rad2Deg;
		...
		public override bool PointCollidesWithShape(Vector2 point, out CollisionResult result) => ShapeCollisions.PointToPoly(point, this, out result);

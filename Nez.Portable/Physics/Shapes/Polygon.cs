...
public class Polygon : Shape
{
    ...
    {
        var index = 0;
        Vector2.Dot(ref points[index], ref direction, out var maxDot);

        for (var i = 1; i < points.Length; i++)
        {
            Vector2.Dot(ref points[i], ref direction, out var dot);
            if (dot > maxDot)
            {
                maxDot = dot;
                index = i;
        ...
        out float distanceSquared, out Vector2 edgeNormal)
    {
        ...
        for (var i = 0; i < points.Length; i++)
        {
            ...
            Vector2.DistanceSquared(ref point, ref closest, out var tempDistanceSquared);
    ...
    internal override void RecalculateBounds(Collider collider)
    {
        ...
        if (collider.ShouldColliderScaleAndRotateWithTransform)
        {
            ...
            if (collider.Entity.Transform.Rotation != 0)
            {
                ...
                var offsetAngle = Mathf.Atan2(collider.LocalOffset.Y * collider.Entity.Transform.Scale.Y,
                    collider.LocalOffset.X * collider.Entity.Transform.Scale.X) * Mathf.Rad2Deg;

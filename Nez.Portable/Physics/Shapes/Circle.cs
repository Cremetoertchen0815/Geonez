	...
	public class Circle : Shape
	{
		...
		internal override void RecalculateBounds(Collider collider)
		{
			...
			if (collider.ShouldColliderScaleAndRotateWithTransform)
			{
				...
				bool hasUnitScale = scale.X == 1 && scale.Y == 1;
				...
				if (collider.Entity.Transform.Rotation != 0)
				{
					...
					float offsetAngle = Mathf.Atan2(collider.LocalOffset.Y, collider.LocalOffset.X) * Mathf.Rad2Deg;
					float offsetLength = hasUnitScale
		...
		public override bool ContainsPoint(Vector2 point) => ((point - position).LengthSquared() <= Radius * Radius);
		...
		public Vector2 GetPointAlongEdge(float angle) => new Vector2(position.X + (Radius * Mathf.Cos(angle)), position.Y + (Radius * Mathf.Sin(angle)));
		...
		/// <returns><c>true</c> if the provided coordinates lie inside this <see cref="Circle"/>; <c>false</c> otherwise.</returns>
		public bool ContainsPoint(float x, float y) => ContainsPoint(new Vector2(x, y));
		...
		public bool ContainsPoint(ref Vector2 point) => (point - position).LengthSquared() <= Radius * Radius;


		public override bool PointCollidesWithShape(Vector2 point, out CollisionResult result) => ShapeCollisions.PointToCircle(point, this, out result);
	}
}
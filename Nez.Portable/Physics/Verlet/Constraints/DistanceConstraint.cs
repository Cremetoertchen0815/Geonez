...
namespace Nez.Verlet
{
	...
	public class DistanceConstraint : Constraint
	{
		...
		{
			float aToCenter = Vector2.Distance(a.Position, center.Position);
			float cToCenter = Vector2.Distance(c.Position, center.Position);
			float distance = Mathf.Sqrt(aToCenter * aToCenter + cToCenter * cToCenter -
		...
		public override void Solve()
		{
			...
			float im1 = 1f / _particleOne.Mass;
			float im2 = 1f / _particleTwo.Mass;
		...
		public override void HandleCollisions(int collidesWithLayers)
		{
			...
			float minX = Math.Min(_particleOne.Position.X, _particleTwo.Position.X);
			float maxX = Math.Max(_particleOne.Position.X, _particleTwo.Position.X);
			float minY = Math.Min(_particleOne.Position.Y, _particleTwo.Position.Y);
			float maxY = Math.Max(_particleOne.Position.Y, _particleTwo.Position.Y);
			...
			{
				if (_polygon.CollidesWithShape(collider.Shape, out var result))
				{
		...
		{
			Vector2 pt;
			for (int j = 0; j < TotalPointsToApproximateCollisionsWith - 1; j++)
			{
				pt = Vector2.Lerp(_particleOne.Position, _particleTwo.Position,
					(j + 1) / (float)TotalPointsToApproximateCollisionsWith);
				int collidedCount = Physics.OverlapCircleAll(pt, 3, VerletWorld._colliders, collidesWithLayers);
				for (int i = 0; i < collidedCount; i++)
				{
					var collider = VerletWorld._colliders[i];
					if (collider.Shape.PointCollidesWithShape(pt, out var collisionResult))
					{

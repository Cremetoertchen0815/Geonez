...
namespace Nez
{
	...
	public class ColliderTriggerHelper
	{
		...
		/// </summary>
		private HashSet<Pair<Collider>> _activeTriggerIntersections = new HashSet<Pair<Collider>>();

		/// <summary>
		/// stores the previous frames intersection pairs so that we can detect exits after moving this frame
		/// </summary>
		private HashSet<Pair<Collider>> _previousTriggerIntersections = new HashSet<Pair<Collider>>();
		private List<ITriggerListener> _tempTriggerList = new List<ITriggerListener>();


		public ColliderTriggerHelper(Entity entity) => _entity = entity;
			...
			for (int i = 0; i < colliders.Count; i++)
			{
				...
				foreach (var neighbor in neighbors)
				{
					...
					if (collider.Overlaps(neighbor))
					{
						...
						bool shouldReportTriggerEvent = !_activeTriggerIntersections.Contains(pair) &&

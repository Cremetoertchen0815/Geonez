...
namespace Nez.Spatial
{
	public class SpatialHash
	{
		...
		private RaycastResultParser _raycastParser;
		...
		/// </summary>
		private Box _overlapTestBox = new Box(0f, 0f);

		/// <summary>
		/// cached circle used for overlap checks
		/// </summary>
		private Circle _overlapTestCirce = new Circle(0f);

		/// <summary>
		/// the Dictionary that holds all of the data
		/// </summary>
		private IntIntDictionary _cellDict = new IntIntDictionary();

		/// <summary>
		/// shared HashSet used to return collision info
		/// </summary>
		private HashSet<Collider> _tempHashset = new HashSet<Collider>();
		...
		private Point CellCoords(int x, int y) => new Point(Mathf.FloorToInt(x * _inverseCellSize), Mathf.FloorToInt(y * _inverseCellSize));
		...
		/// <param name="y">The y coordinate.</param>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private Point CellCoords(float x, float y) => new Point(Mathf.FloorToInt(x * _inverseCellSize), Mathf.FloorToInt(y * _inverseCellSize));
		...
		public void RemoveWithBruteForce(Collider obj) => _cellDict.Remove(obj);


		public void Clear() => _cellDict.Clear();
		...
		{
			for (int x = GridBounds.X; x <= GridBounds.Right; x++)
			{
				for (int y = GridBounds.Y; y <= GridBounds.Bottom; y++)
				{
					var cell = CellAtPosition(x, y);
		...
		private void DebugDrawCellDetails(int x, int y, int cellCount, float secondsToDisplay = 0.5f, float textScale = 1f)
		{
		...
		public HashSet<Collider> GetAllObjects() => _cellDict.GetAllObjects();
		...
		public int Linecast(Vector2 start, Vector2 end, RaycastHit[] hits, int layerMask)
		{
			...
			float xStep = stepX < 0 ? 0f : stepX;
			float yStep = stepY < 0 ? 0f : stepY;
			float nextBoundaryX = (currentCell.X + xStep) * _cellSize;
			float nextBoundaryY = (currentCell.Y + yStep) * _cellSize;
			...
			float tMaxX = ray.Direction.X != 0 ? (nextBoundaryX - ray.Start.X) / ray.Direction.X : float.MaxValue;
			float tMaxY = ray.Direction.Y != 0 ? (nextBoundaryY - ray.Start.Y) / ray.Direction.Y : float.MaxValue;

			// how far do we have to walk before crossing a cell from a cell boundary. may be infinite for near vertical/horizontal rays
			float tDeltaX = ray.Direction.X != 0 ? _cellSize / (ray.Direction.X * stepX) : float.MaxValue;
			float tDeltaY = ray.Direction.Y != 0 ? _cellSize / (ray.Direction.Y * stepY) : float.MaxValue;
	...
	{
		private Dictionary<long, List<Collider>> _store = new Dictionary<long, List<Collider>>();
		...
		/// <param name="y">The y coordinate.</param>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private long GetKey(int x, int y) => unchecked((long)x << 32 | (uint)y);


		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void Add(int x, int y, List<Collider> list) => _store.Add(GetKey(x, y), list);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public bool TryGetValue(int x, int y, out List<Collider> list) => _store.TryGetValue(GetKey(x, y), out list);
	...
	internal class RaycastResultParser
	{
		...
		private static Comparison<RaycastHit> compareRaycastHits = (a, b) => { return a.Distance.CompareTo(b.Distance); };

		//int _cellSize;
		//Rectangle _hitTesterRect; see note in checkRayIntersection
		private RaycastHit[] _hits;
		private RaycastHit _tempHit;
		private List<Collider> _checkedColliders = new List<Collider>();
		private List<RaycastHit> _cellHits = new List<RaycastHit>();
		private Ray2D _ray;
		...
		public bool CheckRayIntersection(int cellX, int cellY, List<Collider> cell)
		{
			for (int i = 0; i < cell.Count; i++)
			{
				...
				if (colliderBounds.RayIntersects(ref _ray, out float fraction) && fraction <= 1.0f)
				{
			...
			for (int i = 0; i < _cellHits.Count; i++)
			{

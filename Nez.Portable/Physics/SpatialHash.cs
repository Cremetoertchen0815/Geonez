...
public class SpatialHash
{
    ...
    /// </summary>
    private readonly HashSet<Collider> _tempHashset = new();
    ...
    public int Linecast(Vector2 start, Vector2 end, RaycastHit[] hits, int layerMask)
    {
        ...
        var xStep = stepX < 0 ? 0f : stepX;
        var yStep = stepY < 0 ? 0f : stepY;
        var nextBoundaryX = (currentCell.X + xStep) * _cellSize;
        var nextBoundaryY = (currentCell.Y + yStep) * _cellSize;
...
{
    private readonly Dictionary<long, List<Collider>> _store = new();
...
internal class RaycastResultParser
{
    ...
    private readonly List<RaycastHit> _cellHits = new();
    private readonly List<Collider> _checkedColliders = new();
    ...
    public bool CheckRayIntersection(int cellX, int cellY, List<Collider> cell)
    {
        for (var i = 0; i < cell.Count; i++)
        {
            ...
            if (colliderBounds.RayIntersects(ref _ray, out var fraction) && fraction <= 1.0f)

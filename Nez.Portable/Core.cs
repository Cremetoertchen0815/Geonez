...
[assembly: InternalsVisibleTo("Nez.ImGui")]
[assembly: InternalsVisibleTo("Nez.Persistence")]
...
public class Core : Game
{
    ...
    private readonly FastList<GlobalManager> _globalManagers = new();
    ...
    internal void FixedUpdate()
    {
        ...
        var timer = DeltaAnalyzer.MeasureSegment("---GLOBAL---", "delta_analyzer",
        ...
        {
            timer = DeltaAnalyzer.MeasureSegment("---GLOBAL---", _globalManagers.Buffer[i].GetType().Name,
                DeltaAnalyzer.DeltaSegmentType.Update);
    ...
    {
        EndDebugUpdate();
        if (SceneTransition == null || (SceneTransition != null &&
                                        (!SceneTransition._loadsNewScene || SceneTransition._isNewSceneLoaded)))
            if (_scene.Enabled)
    ...
    public static void Stall(Action task)
    {
        _instance._timerManager.Stall(task);
    }

    public static void StallTwice(Action task)
    {
        _instance._timerManager.Stall(() => Stall(task));
    }

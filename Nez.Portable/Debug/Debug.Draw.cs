...
namespace Nez
{
	public static partial class Debug
	{
		...
		private static List<DebugDrawItem> _debugDrawItems = new List<DebugDrawItem>();
		private static List<DebugDrawItem> _screenSpaceDebugDrawItems = new List<DebugDrawItem>();
		...
		internal static void Render()
		{
			if (_debugDrawItems.Count > 0)
			{
				...
				for (int i = _debugDrawItems.Count - 1; i >= 0; i--)
				{
			...
			if (_screenSpaceDebugDrawItems.Count > 0)
			{
				...
				for (int i = _screenSpaceDebugDrawItems.Count - 1; i >= 0; i--)
				{
		...
		[Conditional("TRACE")]
		public static void DrawText(string text, float duration = 0) => DrawText(text, Colors.DebugText, duration);

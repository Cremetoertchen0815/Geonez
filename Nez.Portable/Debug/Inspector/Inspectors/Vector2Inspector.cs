...
namespace Nez
{
	public class Vector2Inspector : Inspector
	{
		...
		public override void Initialize(Table table, Skin skin, float leftCellWidth)
		{
			...
			{
				if (float.TryParse(str, NumberStyles.Float, CultureInfo.InvariantCulture, out float newX))
				{
			...
			{
				if (float.TryParse(str, NumberStyles.Float, CultureInfo.InvariantCulture, out float newY))
				{
					var newValue = GetValue<Vector2>();
					newValue.Y = newY;
					SetValue(newValue);

...
namespace Nez.Console
{
	public partial class DebugConsole
	{
		...
		/// </summary>
		private Action[] _functionKeyActions;
		...
		private List<string> _drawCommands;
		...
		private static Regex _charFilter = new Regex(@"[^a-zA-Z0-9!@#$%^&*()_+\-=\[\]{};':""\\|,.<>\/? \r\n]+", RegexOptions.Compiled);
#if TRACE
		...
		public void Log(Exception e)
		{
			...
			{
				string lineWithoutPath = Regex.Replace(line, @"in\s\/.*?\/.*?(\w+\.cs)", "$1");
				Log(lineWithoutPath);
			}
		}
		...
		public void Log(string str)
		{
			...
			int maxWidth = Core.GraphicsDevice.PresentationParameters.BackBufferWidth - 40;
			...
			int maxCommands = (screenHeight - 100) / 30;
		...
		{
			string[] data = _currentText.Split(new char[] { ' ', ',' }, StringSplitOptions.RemoveEmptyEntries);
		...
		internal void Render()
		{
			...
			int screenWidth = Screen.BackbufferWidth;
			int screenHeight = Screen.BackbufferHeight;
			int workingWidth = screenWidth - 2 * HORIZONTAL_PADDING;
			...
			string commandLineString = "> " + _currentText;
			if (_underscore)
				commandLineString += "_";
			...
			if (_drawCommands.Count > 0)
			{
				...
				float height = LINE_HEIGHT * RenderScale * _drawCommands.Count;
				height += (_drawCommands.Count + 1) * TEXT_PADDING_Y;

				float topOfHistoryRect = commandEntryRect.Y - height - COMMAND_HISTORY_PADDING;
				Graphics.Instance.Batcher.DrawRect(HORIZONTAL_PADDING, topOfHistoryRect, workingWidth, height,
					Color.Black * OPACITY);

				float yPosFirstLine = topOfHistoryRect + height - TEXT_PADDING_Y - LINE_HEIGHT * RenderScale;
				for (int i = 0; i < _drawCommands.Count; i++)
				{
					float yPosCurrentLineAddition = (i * LINE_HEIGHT * RenderScale) + (i * TEXT_PADDING_Y);
		...
		public static void BindActionToFunctionKey(int functionKey, Action action) => Instance._functionKeyActions[functionKey - 1] = action;
		...
		public static void BindCommandToFunctionKey(int functionKey, string command, params string[] args) => Instance._functionKeyActions[functionKey - 1] = () => Instance.ExecuteCommand(command, args);
		...
		private void BuildCommandsList()
		{
			...
			try
			{
				...
				object domain = appDomainType.GetRuntimeProperty("CurrentDomain").GetMethod.Invoke(null, Array.Empty<object>());
				...
				object[] methodCallParams = assembliesMethod.GetParameters().Length == 0
					? Array.Empty<object>()
					: new object[] { false };

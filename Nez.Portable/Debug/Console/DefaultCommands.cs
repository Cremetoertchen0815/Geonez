    ...
    public partial class DebugConsole
    {
        ...
        private static void Console(string command = "", string param = "")
        {
            ...
            switch (command)
            {
                ...
                case "scale":
                    if (param == "show")
                    {
                        builder.AppendLine(string.Format("Console scale: {0}", RenderScale));
                        break;
                    }

                    float f;
                    if (float.TryParse(param, out f)) RenderScale = Mathf.Clamp(f, 0.2f, 5f);
                    else RenderScale = 1F;
                    break;
                case "close":
                    Instance.IsOpen = false;
                ...
                default:
                    ...
                    builder.AppendLine(
                        "Sets the scale that the console is rendered. Defaults to 1 and has a max of 5.");
                    builder.AppendLine();
        ...
        private static void TimeCmd(string command = "", string param = "")
        {
            ...
            switch (command)
            {
                case "mode":
                    TimeMode t;
                    if (Enum.TryParse(param, out t)) Time.Mode = t;
                    break;
                case "scale":
                    if (param == "show")
                    {
                        builder.AppendLine(string.Format("Time scale: {0}", Time.TimeScale));
                        break;
                    }

                    float f;
                    if (float.TryParse(param, out f)) Time.TimeScale = f;
                    else RenderScale = 1F;
                    break;
                case "step":
                    if (param == "show")
                    {
                        builder.AppendLine(string.Format("Time step: {0}", 1 / Time.TargetTimeStep));
                        break;
                    }

                    if (float.TryParse(param, out f)) Time.TargetTimeStep = 1 / f;
                    else Time.TargetTimeStep = 1 / 60F;
                    break;
                case "analyzer":
                    bool b;
        ...
        private static void GrphxCmd(string command = "", string param = "")
        {
            ...
            switch (command)
            {
                case "vsync":
                    if (param == "show")
                    {
                        builder.AppendLine(Screen.SynchronizeWithVerticalRetrace.ToString());
                        break;
                    }

                    bool b;
                    if (bool.TryParse(param, out b)) Screen.SynchronizeWithVerticalRetrace = b;
                    else Screen.SynchronizeWithVerticalRetrace = true;
                    break;
                case "drawlog":
                    ...
                    {
                        _drawCallTimer = Core.Schedule(f, true,
                            timer => { Debug.Log("Draw Calls: {0}", Core.drawCalls); });
                    }
                ...
                case "framegraph":
                    if (param == "show")
                    {
                        builder.AppendLine(FramerateGraph.Active.ToString());
                        break;
                    }
                ...
                case "metrics":
                    if (param == "show")
                    {
                        builder.AppendLine(MetricsDisplay.Active.ToString());
                        break;
                    }
                ...
                case "renderable-log":
                    ...
                    for (var j = 0; j < Core.Scene.RenderableComponents.Count; j++)
                    {
                ...
                case "debug":
                    if (param == "show")
                    {
                        builder.AppendLine(string.Format("Debug rendering {0}",
                ...
                default:
                    ...
                    builder.AppendLine(
                        "Enables framerate syncronisation with the vertical blanking. Defaults to true.");
                    builder.AppendLine();
            ...
            switch (command)
            {
                case "count":
                    ...
                    if (!int.TryParse(param, out nr) || nr < 0)
                        builder.AppendLine("Total entities: " + Core.Scene.Entities.Count);
                    else
                        builder.AppendLine("Total entities with tag [" + param + "] " +
                                           Core.Scene.FindEntitiesWithTag(nr).Count);

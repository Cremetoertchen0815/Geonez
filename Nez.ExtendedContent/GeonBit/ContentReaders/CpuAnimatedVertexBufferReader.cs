            ...
            foreach (var channel in channels)
                switch (channel.VertexElementUsage)
                {
                    case VertexElementUsage.Position:
                        Debug.Assert(channel.VertexElementFormat == VertexElementFormat.Vector3);
                    ...
                    case VertexElementUsage.TextureCoordinate:
                        Debug.Assert(channel.VertexElementFormat == VertexElementFormat.Vector2);
                        var tex = input.ReadVector2();
                        gpuVertices[i].TextureCoordinate = tex;
                        break;

                    case VertexElementUsage.BlendWeight:
                        Debug.Assert(channel.VertexElementFormat == VertexElementFormat.Vector4);
                        var wei = input.ReadVector4();
                        cpuVertices[i].BlendWeights = wei;
                        break;

                    case VertexElementUsage.BlendIndices:
                        Debug.Assert(channel.VertexElementFormat == VertexElementFormat.Byte4);

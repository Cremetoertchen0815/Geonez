...
public class DefaultInputProvider : IMouseInput, IKeyboardInput
{
    ...
    private readonly Keys[] _allKeyValues;
    ...
    private float _keyboardInputCooldown;
    ...
    {
        _allKeyValues = (Keys[])Enum.GetValues(typeof(Keys));
        ...
        MousePosition = new Vector2(_newMouseState.X, _newMouseState.Y);
    ...
    public string GetTextInput(string txt, int lineWidth, ref int pos)
    {
        ...
        if (!_newKeyIsPressed && _keyboardInputCooldown > 0f) return txt;

        // if no valid characters are currently input
        if (_currCharacterInput == (char)SpecialChars.Null) return txt;
    ...
    public void UpdateMousePosition(Vector2 pos)
    {
        ...
        MousePosition = _oldMousePos = pos;
    }
    ...
    public Vector2 MousePositionDiff => MousePosition - _oldMousePos;
    ...
    private void NewKeyTextInput(Keys key)
    {
        ...
        var isShiftDown = _newKeyboardState.IsKeyDown(Keys.LeftShift) || _newKeyboardState.IsKeyDown(Keys.RightShift);

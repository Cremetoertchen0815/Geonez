...
/// <summary>
///     GeonBit.UI is part of the GeonBit project, and provide a simple yet extensive UI framework for MonoGame based
///     projects.
...
/// <returns></returns>
public delegate Paragraph DefaultParagraphGenerator(string text, Anchor anchor, Color? color = null,
    float? scale = null, Vector2? size = null, Vector2? offset = null);
...
public class UserInterface : IDisposable
{
    ...
    ///     For example, trying to select a value that doesn't exist from a list would do nothing instead of throwing
    ///     exception.
    /// </summary>
#if RELEASE
    ...
    ///     GeonBit.UI entities use this method when need to create a paragraph, so you can override this to change which
    ///     paragraph type the built-in
    ...
    public static DefaultParagraphGenerator DefaultParagraph =
        (text, anchor, color, scale, size, offset) =>
        {
            if (color != null) return new MulticolorParagraph(text, anchor, color.Value, scale, size, offset);
    ...
    /// <summary>
    ///     Callback to execute every time a new entity is spawned (note: spawn = first time Update() is called on this
    ///     entity).
    /// </summary>
    ...
    private static bool _LockCursorPosition;

    /// <summary>
    ///     Lock the mouse cursor position so it's only moveable with gamepad (need to set AFTER Initilization of
    ///     UserInterface!).
    /// </summary>
    ...
    /// <param name="theme">
    ///     Which UI theme to use (see options in Content/GeonBit.UI/themes/). This affect the appearance of
    ///     all textures and effects.
    /// </param>
    ...
    public static void Initialize(ContentManager contentManager, BuiltinThemes theme)
    {
    ...
    public UserInterface()
    {
        // sanity test
        if (_content == null)
            throw new InvalidStateException("Cannot create a UserInterface before calling UserInterface.Initialize()!");
    ...
    {
        Input.UpdateCursorPosition(position);
    }
    ...
    ///     Width, in pixels to draw the cursor. Height will be calculated automatically to fit texture
    ///     propotions.
    /// </param>
    /// <param name="offset">
    ///     Cursor offset from mouse position (if not provided will draw cursor with top-left corner on mouse
    ///     position).
    /// </param>
    ...
    public void DrawCursor(SpriteBatch spriteBatch)
    {
        ...
        spriteBatch.Begin(SpriteSortMode.Deferred, BlendState, SamplerState, DepthStencilState.None,
            RasterizerState.CullCounterClockwise);

        // calculate cursor size
        var cursorSize = CursorScale * GlobalScale * (_cursorWidth / (float)_cursorTexture.Width);
    ...
    /// <summary>
    ///     Allows you to add a "stained canvas", a customly post-processed copy of the screen that can be used as background
    ///     for entites.
    /// </summary>
    /// <param name="nr"></param>
    ...
    public Texture2D GetCanvasTexture(int nr)
    {
        return _stainedCanvasesTargets[nr];
    }
    ...
    /// <summary>
    ///     Update the UI manager. This function should be called from your Game 'Update()' function, as early as possible (eg
    ///     before you update your game state).
    /// </summary>
    ...
    public void Update(GameTime gameTime)
    {
        ...
        if (_dragTarget != null && !Input.MouseButtonHeldDown()) _dragTarget = null;
        ...
        // set active entity
        if (Input.MouseButtonHeldDown() || Input.GamePadButtonHeldDown(GamePadButton.A_Button)) ActiveEntity = target;
    ...
    {
        var newScreenWidth = spriteBatch.GraphicsDevice.Viewport.Width;
        var newScreenHeight = spriteBatch.GraphicsDevice.Viewport.Height;
        ...
        if (UseRenderTarget)
        {
            ...
            if (StainedCanvasEnabled)
                for (var i = 0; i < _stainedCanvasesPPFX.Count; i++)
                {
            ...
            if (RenderTarget == null ||
                RenderTarget.Width != ScreenWidth ||
                RenderTarget.Height != ScreenHeight)
            {
                // recreate render target
                DisposeRenderTarget();
        ...
        // reset render target
        if (UseRenderTarget) spriteBatch.GraphicsDevice.SetRenderTarget(null);
    }
    ...
    public void DrawMainRenderTarget(SpriteBatch spriteBatch, Rectangle screenSpace)
    {
        ...
        // draw cursor
        if (ShowCursor && !IncludeCursorInRenderTarget) DrawCursor(spriteBatch);
    }

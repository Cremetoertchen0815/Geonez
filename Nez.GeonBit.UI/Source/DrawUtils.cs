...
public class DrawUtils
{
    ...
    public void PushRenderTarget(RenderTarget2D target)
    {
        ...
        if (!UserInterface.Active.UseRenderTarget)
            throw new InvalidStateException(
                "UserInterface.Active.UseRenderTarget must be 'true' to use render-target features!");
    ...
    public virtual void DrawImage(SpriteBatch spriteBatch, Texture2D texture, Rectangle destination,
        Color? color = null, float scale = 1f, Rectangle? sourceRect = null)
    {
        // default color
    ...
    /// <param name="texture">Texture to draw.</param>
    /// <param name="destination">Destination rectangle.</param>
    /// <param name="textureFrameWidth">
    ///     Frame width in percents relative to texture file size. For example, 0.1, 0.1 means the
    ///     frame takes 10% of the texture file.
    /// </param>
    /// <param name="scale">Optional scale factor.</param>
    /// <param name="color">Optional color tint.</param>
    /// <param name="frameScale">Optional scale factor for the frame parts.</param>
    public virtual void DrawSurface(SpriteBatch spriteBatch, Texture2D texture, Rectangle destination,
        Vector2 textureFrameWidth, float scale = 1f, Color? color = null, float frameScale = 1f)
    {
        // default color
        color = FixColorOpacity(color);
        ...
        var ScaleFactor = UserInterface.Active.GlobalScale * frameScale;
        ...
        var needTopBottomStrips = destination.Width > frameSizeDest.X * 2;
        if (needTopBottomStrips)
        {
            ...
            do
            {
                ...
                var toCut = destRect.Right - (destination.Right - frameSizeDest.X);
                if (toCut > 0)
                {
        ...
        var needSideStrips = destination.Height > frameSizeDest.Y * 2;
        if (needSideStrips)
        {
            // current y position
            ...
            do
            {
                ...
                var toCut = destRect.Bottom - (destination.Bottom - frameSizeDest.Y);
                if (toCut > 0)
                {
        ...
        if (needTopBottomStrips && needSideStrips)
        {
            ...
            do
            {
                ...
                var toCutX = destRect.Right - (destination.Right - frameSizeDest.X);
                if (toCutX > 0)
                {
                ...
                do
                {
                    ...
                    // special case - if its last call and this segment overflows bottom, cut it
                    var toCutY = destRect.Bottom - (destination.Bottom - frameSizeDest.Y);
                    if (toCutY > 0)
                    {
    ...
    ///     Frame width in percents relative to texture file size. For example, 0.1 means the frame takes
    ///     10% of the texture file.
    /// </param>
    ...
    public virtual void DrawSurfaceHorizontal(SpriteBatch spriteBatch, Texture2D texture, Rectangle destination,
        float frameWidth, Color? color = null, float frameScale = 1f)
    {
        // default color
        color = FixColorOpacity(color);

        // calc frame size in texture file (Src) and in destination render rect (Dest)
        var ScaleXfac = destination.Height / (float)texture.Height;
        ...
        var frameSizeDest =
            VectorToRoundPoint(new Vector2(frameSizeTextureVector.X * ScaleXfac * frameScale, destination.Height));
        ...
        var centerSizeSrc = VectorToRoundPoint(frameCenterSrcVec);
        var centerSizeDest =
            VectorToRoundPoint(new Vector2(destination.Width - frameSizeDest.X * 2, destination.Height));
    ...
    /// <param name="color">Optional tint color to draw texture with.</param>
    /// <param name="frameScale">Optional scale for the frame part.</param>
    public virtual void DrawSurfaceVertical(SpriteBatch spriteBatch, Texture2D texture, Rectangle destination,
        float frameWidth, Color? color = null, float frameScale = 1f)
    {
        // default color
        color = FixColorOpacity(color);
        ...
        // calc frame size in texture file (Src) and in destination render rect (Dest)
        var ScaleYfac = destination.Width / (float)texture.Width;
        var frameSizeTextureVector = new Vector2(texture.Width, texture.Height * frameWidth);
        var frameSizeSrc = VectorToRoundPoint(frameSizeTextureVector);
        var frameSizeDest =
            VectorToRoundPoint(new Vector2(destination.Width, frameSizeTextureVector.Y * ScaleYfac * frameScale));

        // calc the surface center in texture file (Src) and for drawing destination (Dest)
        var frameCenterSrcVec = new Vector2(texture.Width, texture.Height - frameSizeSrc.Y * 2);
        var centerSizeSrc = VectorToRoundPoint(frameCenterSrcVec);
        var centerSizeDest =
            VectorToRoundPoint(new Vector2(destination.Width, destination.Height - frameSizeDest.Y * 2));

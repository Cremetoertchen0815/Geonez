...
namespace Nez.GeonBit.UI.Entities
{
	...
	public class DynamicParagraph : Entity
	{
		...
		public DynamicParagraph(Func<string> text, Anchor anchor = Anchor.Auto, Vector2? size = null, Vector2? offset = null, float? scale = null) :
		...
		/// <param name="size">Paragraph size (note: not font size, but the region that will contain the paragraph).</param>
		/// <param name="offset">Offset from anchor position.</param>
		public DynamicParagraph(Func<string> text, Anchor anchor, Color color, float? scale = null, Vector2? size = null, Vector2? offset = null) :

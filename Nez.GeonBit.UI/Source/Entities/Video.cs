...
public class VideoBox : Entity
{
    ...
    [XmlIgnore] private VideoPlayer _player = new();
    ...
    static VideoBox()
    {
        MakeSerializable(typeof(VideoBox));
    }
    ...
    public VideoBox(Video video, Vector2? size = null, ImageDrawMode drawMode = ImageDrawMode.Stretch,
    ...
    public Point GetTextureCoordsAt(Vector2 pos)
    {
        ...
        if (DrawMode != ImageDrawMode.Stretch)
            throw new InvalidStateException("Cannot get texture coords on Video that is not in stretched mode!");
    ...
    public Color GetColorAt(Point textureCoords)
    {
        ...
        var index = textureCoords.X + textureCoords.Y * Video.Width;
        _player.GetTexture().GetData(data);
        return data[index];
    }
    ...
    public void CalcAutoWidth()
    {
        ...
        var width = _destRect.Height / (float)Video.Height * Video.Width;
    ...
    {
        UpdateDestinationRectsIfDirty();
        var height = _destRect.Width / (float)Video.Width * Video.Height;
    ...
    protected override void DrawEntity(SpriteBatch spriteBatch, DrawPhase phase)
    {
        ...
        switch (DrawMode)
        {
            ...
            case ImageDrawMode.Stretch:
                UserInterface.Active.DrawUtils.DrawImage(spriteBatch, frame, _destRect, FillColor, Scale,
                    SourceRectangle);
                break;
        }

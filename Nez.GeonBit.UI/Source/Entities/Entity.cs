...
namespace Nez.GeonBit.UI.Entities
{
	...
	public abstract class Entity
	{
		...
		/// <summary>
		/// Is the entity currently selectable (use it to define entities, which should / should not be selectable by the gamepad).
		/// </summary>
		...
		/// </summary>
		private bool _isFirstSelection = false;
		/// <summary>
		...
		/// </summary>
		private bool _doFirstSelection = false;
		/// <summary>
		...
		private bool _isFirstAfterDraw = false;
		...
		private bool _selectionOnDstCalc { get; set; } = false;
				...
				if (sibling.Visible && sibling.Selectable && sibling.Enabled)
				{
					...
					if (entity.AttachedData != null && entity.AttachedData is TabData)
					{
							...
							((TabData)entity.AttachedData).button.AttachedData is PanelTabs)
						{
							...
							((PanelTabs)((TabData)entity.AttachedData).button.AttachedData).
								_children[0]._children.ForEach(a => a.IsFirstSelection = false);

							// revert the IsFirstSelection status for the current active tab
							((PanelTabs)((TabData)entity.AttachedData).button.AttachedData).
								ActiveTab.button.IsFirstSelection = true;
		...
		private Entity RecursiveFindFirstSelection(Entity rootEntity)
		{
			...
			if (rootEntity.IsFirstSelection)
			{
				return rootEntity;
			}
		...
		private void SelectNextEntity(Entity parent)
		{
			if (Parent != null && Parent._children != null && Parent is RootPanel == false)
			{
				...
				{
					currentEntity = RecursiveFindFirstSelection(UserInterface.Active.Root);
				}

				if (currentEntity == null)
				{
					throw new System.Exception("No GamePad entry point was found. At least one entity in a panel must have the 'IsFirstSelection' flag.");
				}

				bool nextSelectionFound = false;
				for (int i = currentEntity._indexInParent + 1; i < parent._children.Count; i++)
				{
				...
				if (!nextSelectionFound)
				{
					SelectFirstEntity(currentEntity.Parent, true);
				}
			}
		}
		...
		private void SelectPreviousEntity(Entity parent)
		{
			if (parent != null && parent._children != null && parent is RootPanel == false)
			{
				...
				bool previousSelectionFound = false;
				for (int i = currentEntity._indexInParent - 1; i >= 0; i--)
				{
						...
						previousEntity.Enabled)
					{
						...
						if (previousEntity is Button)
						{
							...
							{
								SetCursorPosition(previousEntity); // if it's not a panel tabs button the set the cursor to it
							}
						}
						else
						{
				...
				{
					SelectLastEntity(currentEntity.Parent, true);
				}
			}
		}
		...
		private void SelectLeftEntity()
		{
			...
			var next = lst.OrderByDescending(a => a._destRect.Right).ThenBy(b => Math.Abs(b._destRect.Center.Y - _destRect.Center.Y)).First();
			next.Select();
			next.CalcDestRect();
			next.IsFocused = true;
		}
		private void SelectRightEntity()
		{
			...
			if (lst.Count < 1) return;
			var next = lst.OrderBy(a => a._destRect.Left).ThenBy(b => Math.Abs(b._destRect.Center.Y - _destRect.Center.Y)).First();
			next.Select();
			next.CalcDestRect();
			next.IsFocused = true;
		}
		private void SelectAboveEntity()
		{
			...
			if (lst.Count < 1) return;
			var next = lst.OrderByDescending(a => a._destRect.Bottom).ThenBy(b => Math.Abs(b._destRect.Center.X - _destRect.Center.X)).First();
			next.Select();
			next.CalcDestRect();
			next.IsFocused = true;
		}
		private void SelectBelowEntity()
		{
			...
			if (lst.Count < 1) return;
			var next = lst.OrderBy(a => a._destRect.Top).ThenBy(b => Math.Abs(b._destRect.Center.X - _destRect.Center.X)).First();
			next.Select();
			next.CalcDestRect();
			next.IsFocused = true;
		}
		...
		private void RecursiveSearch(List<Entity> entities, Entity current, Func<Entity, bool> predicate)
		{
			...
			if (current.Selectable && current.Visible && predicate(current) && current is Panel == false) entities.Add(current);
		}
		...
		public virtual void Update(ref Entity targetEntity, ref Entity dragTargetEntity, ref bool wasEventHandled, Point scrollVal)
		{
			...
			if (targetEntity == this)
			{
				...
				if (_entityState == EntityState.MouseDown && UserInterface.GetCursorMode == UserInterface.CursorMode.Roaming)
				{
					DoWhileMouseDown();
				}
				else

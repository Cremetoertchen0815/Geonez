...
namespace Nez.GeonBit.UI.Entities
{
	...
	public class Slider : Entity
	{
		...
		protected override void DrawEntity(SpriteBatch spriteBatch, DrawPhase phase)
		{
			...
			float ScaleXfac = _destRect.Height / frameSizeTexture.Y;
			...
			if (UseDottedDesign)
			{
				for (uint i = Min; i <= Max; i++)
				{
					...
					float posX = _destRect.X + _frameActualWidth + (_destRect.Width - _frameActualWidth * 2) * (((float)i - Min) / ((float)Max - Min));

					MarkRec = new Rectangle((int)(System.Math.Round(posX) - frameSizeTexture.X / 2f), _destRect.Y + (int)(frameSizeTexture.Y * 0.5f), (int)frameSizeTexture.X, (int)frameSizeTexture.Y);
					UserInterface.Active.DrawUtils.DrawImage(spriteBatch, texture, MarkRec, FillColor);
				}

				// now draw mark
				var perc = GetValueAsPercent();
				float markX = _destRect.X + _frameActualWidth + (_destRect.Width - _frameActualWidth * 2) * perc;

				MarkRec = new Rectangle((int)System.Math.Round(markX) - _markWidth / 2, _destRect.Y, _markWidth, markHeight);
		...
		public void DoOnGamePadMove()
		{
			...
			int testValue = _value + stepDirection * GetStepSize();
